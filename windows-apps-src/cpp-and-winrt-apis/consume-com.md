...
Wenn Sie mit der COM-Programmierung, arbeiten Sie direkt mit Schnittstellen anstatt mit Objekten (die auch im Hintergrund für Windows-Runtime-APIs, die eine Weiterentwicklung des COM sind "true" des). Um eine Funktion für eine COM-Klasse aufrufen, z. B. Sie aktivieren die Klasse, eine Schnittstelle zurück, und rufen Sie dann Sie Funktionen auf diese Schnittstelle. Der Zustand eines Objekts für den Zugriff auf zugreifen nicht Sie Datenelemente direkt. Rufen Sie stattdessen Accessor und Zugriffsfunktion Funktionen in einer Schnittstelle.
...
Der obige Code zeigt, wie einen nicht initialisierten intelligenten Zeiger auf eine [**ID2D1Factory1**](https://msdn.microsoft.com/library/Hh404596) COM-Schnittstelle deklarieren. Der intelligente Zeiger ist nicht initialisiert, damit es noch nicht auf eine **ID2D1Factory1** -Schnittstelle zu tatsächlichen Objekt (es zeigt nicht auf eine Schnittstelle überhaupt) gehören verwiesen wird. Es besitzt das Potenzial dazu jedoch. und hat (wobei ein intelligenter Zeiger) die Möglichkeit, über COM-referenzzählung zum Verwalten der Lebensdauer des besitzenden Objekts der Schnittstelle, die darauf verweist, und die Mittel, Sie Funktionen für diese Schnittstelle aufrufen.
...
Um zu prüfen, dass der Wert für eine HRESULT von einer COM-Funktion zurückgegeben und löst eine Ausnahme aus, wenn es sich um einen Fehlercode darstellt, rufen Sie [**WinRT:: check_hresult**](/uwp/cpp-ref-for-winrt/error-handling/check-hresult).
...
Sie können die kostenlose [**winrt::get_unknown**](/uwp/cpp-ref-for-winrt/windows-foundation-iunknown#getunknown-function) -Funktion, um Ihre **Com_ptr** an eine Funktion übergeben, die einen **IUnknown** -Schnittstellenzeiger akzeptiert aufrufen.
...
Funktion, die einen intelligenten COM-Zeiger in Form einer **WinRT:: com_ptr** sollten als konstanten Verweis oder durch einen Verweis tun.
...
Sie können die [**com_ptr:: As**](/uwp/cpp-ref-for-winrt/com-ptr#comptras-function) Funktion verwenden, einen intelligenten COM-Zeiger für eine andere Schnittstelle Abfragen. Die Funktion löst eine Ausnahme aus, wenn die Abfrage nicht erfolgreich.
...
Alternativ verwenden [**com_ptr::try_as**](/uwp/cpp-ref-for-winrt/com-ptr#comptrtryas-function), die einen Wert zurückgibt, die Sie gegen überprüfen kann `nullptr` zu überprüfen, ob die Abfrage erfolgreich war.
...
Wenn Sie erstellen und diese Quelle Codebeispiel wird anschließend die erste, in Visual Studio ausführen möchten, erstellen Sie ein neues **Core App (C++ / WinRT)**. `Direct2D` der name kann sich nichts anderes ist ein angemessener Namen für das Projekt. Open `App.cpp`, löschen Sie den gesamten Inhalt, und fügen Sie in den folgenden Eintrag.
...
Wie Sie sehen können, C++ / WinRT stellt Unterstützung für das Implementieren und Aufrufen von COM-Schnittstellen. Für die Verwendung von COM-Typen, wie und VARIANT, ist immer die Möglichkeit, diese in ihrer unformatierten Form (zusammen mit der entsprechenden APIs). Alternativ können Sie die Wrapper durch ein Framework, z. B. die [Active Template Library (ATL)](/cpp/atl/active-template-library-atl-concepts), durch die Visual C++-Compiler- [COM-Unterstützung](/cpp/cpp/compiler-com-support), oder auch durch Ihre eigenen Wrapper bereitgestellt.

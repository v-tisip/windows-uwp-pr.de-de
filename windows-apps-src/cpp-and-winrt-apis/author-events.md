...
Wie oben zu sehen ist, wird das Ereignis in Bezug auf die [**winrt::event**](/uwp/cpp-ref-for-winrt/event) strukturvorlage eines bestimmten Delegattyps als Parameter implementiert.
...
Jedes Mal, wenn Sie auf das Fenster klicken, ziehen Sie 1 vom Kontostand ab. Um zu zeigen, dass das Ereignis wie erwartet ausgelöst wird, setzen Sie einen Haltepunkt in der Lambda-Ausdruck, der das Ereignis **AccountIsInDebit** verarbeitet, führen Sie die app und klicken Sie auf der Fenstergröße.

## <a name="parameterized-delegates-and-simple-signals-across-an-abi"></a>Parametrisierten Delegaten und einfache Signale, über einen ABI
...
Die Typparameter für diese beiden Delegattypen müssen die ABI überschreiten, daher die Typparameter müssen Windows-Runtime-Typen zu sein. Erst- und Drittanbieter-Laufzeitklassen als auch primitive Typen wie z. B. Zahlen und Zeichenfolgen enthält. Der Compiler hilft Ihnen mit dem Fehler "*WinRT-Typ*", wenn Sie diese Einschränkung vergessen.

Wenn Sie keine Parameter oder Argumente mit dem Ereignis übergeben müssen, können Sie Ihre eigenen einfache Windows-Runtime-Delegattyp definieren. Das folgende Beispiel zeigt eine einfachere Version der Laufzeitklasse **BankAccount** . Delegattyp **SignalDelegate** deklariert, und klicken Sie dann es verwendet, um ein Ereignis Signal-Typ anstelle eines Ereignisses mit einem Parameter auszulösen.
...
Das folgende Beispiel zeigt zunächst einen Delegaten Signatur, die keine Parameter (im Wesentlichen eine einfache Signal) und dann ein, die eine Zeichenfolge übernimmt.
...
Beachten Sie, wie Sie das Ereignis hinzufügen können beliebig viele abonnierende Delegaten wie gewünscht. Es gibt jedoch einige Aufwand mit einem Ereignis. Wenn Sie benötigen lediglich einen einfachen Rückruf mit nur einem einzelnen abonnierenden Delegaten, Sie können [**WinRT:: Delegate&lt;… T&gt; **](/uwp/cpp-ref-for-winrt/delegate) eigenständig.

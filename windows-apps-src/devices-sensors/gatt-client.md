...
- Abonnieren Sie für Benachrichtigungen, wenn über ändert Wert
...
Bluetooth LE-GATT-APIs machen Objekte und Funktionen verfügbar, statt den Zugriff auf den unformatierten Transport. Die GATT-APIs ermöglichen auch Entwicklern arbeiten mit Bluetooth LE-Geräten in der Lage, die folgenden Aufgaben ausführen:
...
```
Andererseits, löschen alle Verweise auf ein BluetoothLEDevice-Objekt für ein Gerät zugeordnet ist (und keine andere app auf dem System einen Verweis auf das Gerät hat) löst ein Automatisches Trennen nach einem kleinen Timeout-Zeitraum. 
...
```
Nachdem der Dienst Orte erkannt wurde, wird der nächste Schritt ist, zum Abfragen, um Eigenschaften. 
...
```  
Das Betriebssystem gibt, die eine schreibgeschützte Liste GattCharacteristic-Objekten, die Sie Vorgänge durchführen können.

## <a name="perform-readwrite-operations-on-a-characteristic"></a>Führen Sie Lese-/Schreibzugriff auf ein Merkmal

Die Eigenschaft ist, dass die grundlegende Einheit der GATT Kommunikation basiert. Sie enthält einen Wert, der einen unterschiedlichen Teil der Daten auf dem Gerät darstellt. Beispielsweise hat die Ebene Akku-Eigenschaft einen Wert, der Akkustand des Geräts darstellt.

Lesen Sie die über Eigenschaften, um zu ermitteln, welche Vorgänge unterstützt werden:
```csharp
...
```
> **Tipp**: Erste Schritte mit dem [DataReader](https://msdn.microsoft.com/en-us/library/windows/apps/windows.storage.streams.datareader.aspx) und [DataWriter](https://msdn.microsoft.com/en-us/library/windows/apps/windows.storage.streams.datawriter.aspx)verwenden. Die Funktionalität wird Anwendungsregistrierungsprüfpunkte, bei der Arbeit mit der unformatierten Puffer, die Sie aus vielen der Bluetooth-APIs zu erhalten. 
## <a name="subscribing-for-notifications"></a>Abonnieren für Benachrichtigungen

Stellen Sie sicher, das Merkmal unterstützt angeben oder benachrichtigen (die über Eigenschaften, um sicherzustellen, überprüfen). 

> **Reservieren**: angeben, gilt als zuverlässiger, da jedes Ereignis Wert hat sich geändert, die mit einer Bestätigung durch den Client-Gerät gekoppelt ist. Benachrichtigen ist mehr verbreitet, da die meisten GATT-Transaktionen würden stattdessen Energie sparen, sondern äußerst zuverlässig. In jedem Fall alle diese erfolgt auf, damit die app nicht beteiligt sich wird. Wir werden gemeinsam auf sie verweisen, als einfach "Notifications", aber Sie wissen jetzt. 

Es gibt zwei Dinge zu erledigen vor dem Abrufen der Benachrichtigungen:
- Schreiben Sie in Client über Konfiguration Deskriptor (CCCD)
- Behandeln Sie das Characteristic.ValueChanged-Ereignis

Schreiben in die CCCD weist dem Server-Gerät, dass dieser Client jedes Mal, bestimmte merkmalswert Änderungen wissen möchte. Gehen Sie dazu folgendermaßen vor: 
...
```
Jetzt wird die GattCharacteristic ValueChanged-Ereignis jedes Mal aufgerufen, die der Wert auf dem Remotegerät geändert wird. Ist den Ereignishandler implementiert ist: 

...
ms.locfileid: "8797117"
---
# <a name="composition-brushes"></a>Kompositionspinsel
Alles, was von einer UWP-Anwendung auf dem Bildschirm sichtbar wird angezeigt, da es mit einem Pinsel gezeichnet wurde. Pinsel können Sie Benutzer Objekte der Benutzeroberfläche (UI) mit Inhalt, angefangen bei einfachen, einfarbige Farben zu Bildern oder Zeichnungen für komplexe Effekte Kette zeichnen. In diesem Thema werden die Begriffe zum Zeichnen mit CompositionBrush.

Beachten Sie bei der Arbeit mit XAML-UWP-app können Sie sich entschieden, ein UIElement mit einem [XAML-Pinsel](/windows/uwp/design/style/brushes) oder ein [CompositionBrush](https://docs.microsoft.com/uwp/api/Windows.UI.Composition.CompositionBrush)zu zeichnen. In der Regel ist es einfacher und ratsam, einen XAML-Pinsel auswählen, ob Ihr Szenario von einem XAML-Pinsel unterstützt wird. Animieren beispielsweise die Farbe einer Schaltfläche, die Füllung ein Text oder eine Form mit einem Bild ändern. Andererseits, wenn Sie versuchen, eine Aktion ausführen, die von einem XAML-Pinsel wie Zeichnen mit eine animierte Maske oder eine animierte neun Raster Stretch oder einer effektkette nicht unterstützt wird, können eine CompositionBrush Sie ein UIElement durch die Verwendung von [Zeichnen XamlCompositionBrushBase](https://docs.microsoft.com/uwp/api/windows.ui.xaml.media.xamlcompositionbrushbase).

Bei der Arbeit mit der visuellen Ebene muss eine CompositionBrush verwendet werden, um den Bereich einer [SpriteVisual](https://docs.microsoft.com/uwp/api/Windows.UI.Composition.SpriteVisual)zeichnen.
...
-   [Zeichnen mit CompositionBrush](./composition-brushes.md#paint-with-a-compositionbrush)
    ...
    -   [Hintergrund-Pixel mit Paint](./composition-brushes.md#paint-using-background-pixels)
...
|[CompositionNineGridBrush](https://docs.microsoft.com/uwp/api/Windows.UI.Composition.CompositionNineGridBrush)      |Zeichnet einen Bereich mit einer mit einem NineGrid Stretch CompositionBrush |Windows 10 Anniversary Update SDK (14393)
...
Die folgende Abbildung und der Code zeigt ein SpriteVisual mit LinearGradientBrush 2 beendet, die mit einer roten und gelben Farbe gezeichnet.
...
Ein [CompositionSurfaceBrush](https://docs.microsoft.com/uwp/api/Windows.UI.Composition.CompositionSurfaceBrush) kann auch verwendet werden, um einen Bereich mit Pixel aus einer ICompositionSurface mithilfe von [Win2D](http://microsoft.github.io/Win2D/html/Introduction.htm) (oder D2D) gerendert zu zeichnen.

Der folgende Code zeigt, dass mit einer gerenderten auf eine ICompositionSurface ausführen ein spritevisual-Elements gezeichnet mithilfe von Win2D. Beachten Sie, um Win2D zu verwenden, Sie in Ihrem Projekt das [Win2D-NuGet](http://www.nuget.org/packages/Win2D.uwp) -Paket einschließen müssen.
...
Auf ähnliche Weise kann die CompositionSurfaceBrush auch zum Zeichnen eines spritevisual-Elements mit einer Spielinhalte mithilfe von Win2D-Interoperabilität verwendet werden. [Dieses Beispiel](https://github.com/Microsoft/Win2D-Samples/tree/master/CompositionExample) enthält ein Beispiel zur Verwendung von Win2D zum Zeichnen eines spritevisual-Elements mit einem Spielinhalte.

### <a name="paint-with-a-video"></a>Zeichnen Sie mit einem video
Ein [CompositionSurfaceBrush](https://docs.microsoft.com/uwp/api/Windows.UI.Composition.CompositionSurfaceBrush) kann auch verwendet werden, um einen Bereich mit Pixel aus einer ICompositionSurface gerendert wird, verwenden ein Video über der [MediaPlayer](https://docs.microsoft.com/en-us/uwp/api/Windows.Media.Playback.MediaPlayer) -Klasse geladen zu zeichnen.

Der folgende Code zeigt, dass ein spritevisual-Element mit einem Video auf eine ICompositionSurface geladen gezeichnet.
...
Eine [CompositionEffectBrush](https://docs.microsoft.com/uwp/api/Windows.UI.Composition.CompositionEffectBrush) zeichnet einen Bereich mit der Ausgabe von einem CompositionEffect. Effekte in der visuellen Ebene können als animierbaren Filtereffekte auf eine Auflistung des Inhalts der Medienquelle z. B. Farben Farbverläufe, Bilder, Videos, Swapchains, Regionen der Benutzeroberfläche und Strukturen von visuellen Elementen angewendet betrachtet werden. Der Inhalt der Quelle wird in der Regel angegeben mit einem anderen CompositionBrush.
...
Weitere Informationen zum Erstellen eines Effekts mit CompositionBrushes finden Sie in der [Effekte in der visuellen Ebene](https://docs.microsoft.com/en-us/windows/uwp/composition/composition-effects)
...
Eine [CompositionMaskBrush](https://docs.microsoft.com/uwp/api/Windows.UI.Composition.CompositionMaskBrush) zeichnet einen Bereich mit einem CompositionBrush mit einer Deckkraftmaske angewendet wird. Die Quelle der die Deckkraftmaske kann alle CompositionBrush des Typs CompositionColorBrush, CompositionLinearGradientBrush, CompositionSurfaceBrush, CompositionEffectBrush oder CompositionNineGridBrush sein. Die Deckkraftmaske muss als eine CompositionSurfaceBrush angegeben werden.

Die folgende Abbildung und der Code zeigt ein Element, das mit einem CompositionMaskBrush SpriteVisual. Die Quelle der Maske ist ein CompositionLinearGradientBrush die maskiert wird, um einen Kreis mit einem Bild der Kreis als Maske aussehen.
...
Eine [CompositionNineGridBrush](https://docs.microsoft.com/uwp/api/Windows.UI.Composition.CompositionNineGridBrush) zeichnet einen Bereich mit einem CompositionBrush, die mit der neun Raster-gestreckt wird. Die neun Raster-können Sie Ränder und Ecken von einem CompositionBrush anders als den Mittelpunkt gestreckt. Die Quelle der das Raster neun Stretch kann, indem alle CompositionBrush des Typs CompositionColorBrush, CompositionSurfaceBrush oder CompositionEffectBrush.

Der folgende Code zeigt, dass ein spritevisual-Element mit einem CompositionNineGridBrush gezeichnet. Die Quelle der Maske ist ein CompositionSurfaceBrush die gestreckt wird mit einem neun-Raster.
...
### <a name="paint-using-background-pixels"></a>Hintergrund-Pixel mit Paint

Eine [CompositionBackdropBrush](https://docs.microsoft.com/uwp/api/Windows.UI.Composition.CompositionBackdropBrush) zeichnet einen Bereich mit den Inhalt hinter den Bereich. Eine CompositionBackdropBrush wird nie eigene verwendet, jedoch stattdessen als Eingabe für eine andere CompositionBrush wie ein EffectBrush verwendet wird. Beispielsweise können Sie einen Effekt Milchglas mithilfe einer CompositionBackdropBrush als Eingabe für einen Weichzeichnereffekt zu erzielen.

Der folgende Code zeigt eine kleine visuelle Struktur, um ein Bild mit CompositionSurfaceBrush und eine Überlagerung Milchglas über dem Bild zu erstellen. Die Überlagerung Milchglas wird durch das Platzieren eines spritevisual-Elements mit einer EffectBrush über dem Bild gefüllt erstellt. Die EffectBrush verwendet eine CompositionBackdropBrush als Eingabe für den Weichzeichnereffekt zu.
...
Verwenden eine Reihe von CompositionBrushes andere CompositionBrushes als Eingaben. Beispielsweise kann mithilfe der Methode SetSourceParameter verwendet werden, legen Sie eine andere CompositionBrush als Eingabe für eine CompositionEffectBrush. Die folgende Tabelle zeigt die unterstützten Kombinationen von CompositionBrushes. Beachten Sie, dass eine nicht unterstützte Kombination eine Ausnahme ausgelöst wird.
...
> [!NOTE]
> Wenn ein CompositionBrush für ein XAML-UIElement empfohlen wird, wird davon ausgegangen, dass die CompositionBrush verpackt ist eine XamlCompositionBrushBase verwenden.

|Szenario                                                                   | XAML-UIElement                                                                                                |SpriteVisual Komposition
|---------------------------------------------------------------------------|---------------------------------------------------------------------------------------------------------------|----------------------------------
...
|Zeichnen Sie einen Bereich mit einem Bild mit NineGrid stretch                         |[Image-Steuerelement](https://docs.microsoft.com/uwp/api/Windows.UI.Xaml.Controls.Image)                   |[CompositionNineGridBrush](https://docs.microsoft.com/uwp/api/Windows.UI.Composition.CompositionNineGridBrush)
...
|Zeichnen eines Bereichs mit benutzerdefinierten 2D zeichnen                                       |[CanvasControl](http://microsoft.github.io/Win2D/html/T_Microsoft_Graphics_Canvas_UI_Xaml_CanvasControl.htm) von Win2D                                                                                                 |[CompositionSurfaceBrush](https://msdn.microsoft.com/library/windows/apps/Mt589415) mit Win2D-Interoperabilität
...
|Zeichnen Sie einen Bereich mit einem Effekt auf Hintergrundpixeln angewendet        |[CompositionBackdropBrush](https://docs.microsoft.com/uwp/api/Windows.UI.Composition.CompositionBackdropBrush)                                                                                        |[CompositionBackdropBrush](https://docs.microsoft.com/uwp/api/Windows.UI.Composition.CompositionBackdropBrush)

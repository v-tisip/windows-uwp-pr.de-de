...
-   Die einmaligen Modus können Sie um ein [**Bild**](https://msdn.microsoft.com/library/windows/apps/BR242752) an das Foto des aktuellen Benutzers zu binden.
...
Damit Sie nicht zum Implementieren des Musters über mehrere Male angezeigt, wenn Sie c# verwenden Sie Sie einfach können ableiten, **BindableBase** Basisklasse, die Sie in das [QuizGame](https://github.com/Microsoft/Windows-appsample-quizgame) -Beispiel (in den Ordner "Common") finden. Dies ist ein Beispiel hierfür.
...
Um dies zu veranschaulichen, dass die **HostViewModel.NextButtonText** -Eigenschaft tatsächlich Observable, fügen Sie einen **Click** -Ereignishandler der Schaltfläche hinzu, und aktualisieren Sie den Wert der **HostViewModel.NextButtonText**. Erstellen, ausführen, und klicken Sie auf die Schaltfläche, um den Wert von der Schaltfläche **Inhalten** zu aktualisieren.
...
> [!NOTE]
> **{X: Bind}** eignet sich nicht für spät gebundene Szenarien, z. B. beim Navigieren in der Wörterbuchstruktur ein JSON-Objekt, noch Duck eingeben. "Duck-typing" ist eine schwache Form der Eingabe basierend auf lexikalischen Übereinstimmungen Eigenschaftennamen (einen "führt, schwimmt und quakt wie eine Ente, dann wird eine Ente"). Mit Duck-typing würde eine Bindung zur **Alter** -Eigenschaft auch mit einer **Person** erfüllt, oder **Wein** -Objekts (vorausgesetzt, dass diese Typen eine **Alter** -Eigenschaft hat). Verwenden Sie für diese Szenarien der Markuperweiterung **{Binding}** .
...
- eine Sammlung von **BookSku**s, deren **AuthorName** -Eigenschaft dem Gruppenschlüssel entspricht.

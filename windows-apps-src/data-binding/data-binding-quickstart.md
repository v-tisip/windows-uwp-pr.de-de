...
Wenn Sie verwenden [C++ / WinRT](/windows/uwp/cpp-and-winrt-apis/intro-to-using-cpp-with-winrt), müssen Sie so entfernen Sie die Funktion **MainPage:: clickHandler** im Auftrag für das Projekt zu erstellen.
...
Wenn Sie verwenden [C++ / WinRT](/windows/uwp/cpp-and-winrt-apis/intro-to-using-cpp-with-winrt), und klicken Sie dann mehr über das Binden an eine Observable-Collection in erfahren Sie können [XAML-items-Steuerelemente; binden an eine C++ / WinRT-Collection](/windows/uwp/cpp-and-winrt-apis/binding-collection). Wenn Sie in diesem Thema zuerst Sie dann die Absicht der C++ lesen / WinRT-Codebeispiel unten wird deutlicher werden.
...
Sie haben zwei Möglichkeiten, dieses Verhalten zu implementieren. Sie können die Detailansicht an die [**SelectedItem**](https://msdn.microsoft.com/library/windows/apps/BR209770)-Eigenschaft der [**ListView**](https://msdn.microsoft.com/library/windows/apps/BR242878) binden. Oder Sie können eine [**CollectionViewSource**](https://msdn.microsoft.com/library/windows/apps/BR209833)verwenden, der in diesem Fall binden Sie sowohl **ListView** als auch die Detailansicht an die **CollectionViewSource** (tun dies der Fall übernimmt Vorsicht das derzeit ausgewählte Elements für Sie). Beide Methoden sind unten aufgeführt, und beide bieten umfassende dieselben Ergebnisse (in der Abbildung gezeigt).
...
Es liegt ein Problem mit dem oben gezeigten Rendering. Die **ReleaseDateTime** -Eigenschaft ist nicht nur ein Datum, es [**DateTime**](/uwp/api/windows.foundation.datetime) (Wenn Sie C++ verwenden, ist es einem [**Kalender**](/uwp/api/windows.globalization.calendar)). Daher wird in c#, es angezeigt präziser als wir benötigen. Und in C++ wird es als Typname gerendert wird. Eine Lösung besteht darin, eine Zeichenfolgeneigenschaft zur **Aufnahme** Klasse hinzufügen, die das Äquivalent zurückgibt `this.ReleaseDateTime.ToString("d")`. Diese Eigenschaft **ReleaseDate** würde angeben, dass sie ein Datum und kein Datum und Uhrzeit zurückgibt. Die Benennung als **ReleaseDateAsString** gibt dann auch noch an, dass sie eine Zeichenfolge zurückgibt.
...
> [!NOTE]
> Ab Windows 10, Version 1607, bietet das XAML-Framework einen integrierten boolescher Wert, Sichtbarkeit Konverter. Der Konverter ordnet der Enumerationswert **Visibility.Visible einstellen** und **"false",** um **Visibility.Collapsed** **"true"** , sodass Sie eine Visibility-Eigenschaft an einen booleschen Wert binden können, ohne einen Konverter zu erstellen. Für die Verwendung des integrierten Konverters muss die SDK-Zielversion der App mindestens 14393 lauten. Die Verwendung ist nicht möglich, wenn Ihre App für frühere Versionen von Windows10 bestimmt ist. Weitere Informationen zu Zielversionen finden Sie in [Version adaptiven Code](https://msdn.microsoft.com/windows/uwp/debug-test-perf/version-adaptive-code).

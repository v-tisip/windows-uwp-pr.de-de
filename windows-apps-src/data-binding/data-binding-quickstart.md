...
Wenn Sie c# oder C++ verwenden / CX, dann Ihr Projekt eine neue Klasse hinzu, und nennen Sie die Klasse **Aufzeichnung**.
...
Wenn Sie verwenden [C++ / WinRT](/windows/uwp/cpp-and-winrt-apis/intro-to-using-cpp-with-winrt), und Sie mehr über das Binden an eine Observable-Collection in erfahren können [XAML-items-Steuerelemente; binden an eine C++ / WinRT-Collection](/windows/uwp/cpp-and-winrt-apis/binding-collection). Wenn Sie dieses Thema zuerst Sie dann die Absicht der C++ lesen / WinRT-Codebeispiel unten wird deutlicher werden.
...
Sie haben zwei Möglichkeiten, dieses Verhalten zu implementieren. Sie können die Detailansicht an die [**SelectedItem**](https://msdn.microsoft.com/library/windows/apps/BR209770)-Eigenschaft der [**ListView**](https://msdn.microsoft.com/library/windows/apps/BR242878) binden. Oder Sie können eine [**CollectionViewSource**](https://msdn.microsoft.com/library/windows/apps/BR209833)verwenden, der in diesem Fall binden Sie sowohl **ListView** als auch die Detailansicht an die **CollectionViewSource** (dabei so dauert Vorsicht das derzeit ausgewählte Elements für Sie). Beide Methoden sind unten aufgeführt, und beide bieten umfassende dieselben Ergebnisse (in der Abbildung gezeigt).
...
> [!NOTE]
> Wenn Sie C++ verwenden, klicken Sie dann Ihre Benutzeroberfläche wird nicht immer genau in der folgenden Abbildung: das Rendern der Eigenschaft **ReleaseDateTime** unterscheidet. Finden Sie im folgenden Abschnitt für Weitere Hinweise dazu.
...
Es liegt ein Problem mit dem oben gezeigten Rendering. Die **ReleaseDateTime** -Eigenschaft ist nicht nur ein Datum, es [**DateTime**](/uwp/api/windows.foundation.datetime) (Wenn Sie C++ verwenden, ist es einem [**Kalender**](/uwp/api/windows.globalization.calendar)). Daher wird in c#, es angezeigt präziser als wir benötigen. Und in C++ wird es als Typname gerendert wird. Eine Lösung besteht darin, eine Zeichenfolgeneigenschaft der **Aufnahme** -Klasse hinzufügen, das Äquivalent zurückgibt `this.ReleaseDateTime.ToString("d")`. Diese Eigenschaft **ReleaseDate** würde angeben, dass sie ein Datum und kein Datum und die Uhrzeit zurückgibt. Die Benennung als **ReleaseDateAsString** gibt dann auch noch an, dass sie eine Zeichenfolge zurückgibt.
...
Wie oben zu sehen ist, verwenden Sie für die Formatierung von Flexibilität wir das Markup eine Formatzeichenfolge in den Konverter durch den Konverterparameter übergeben. Verwenden Sie in den Codebeispielen in diesem Thema nur die C#-Wertkonverter dargestellt, macht dieses Parameters. Aber konnte problemlos eine C++-Style-Formatzeichenfolge als der Konverterparameter übergeben, und verwenden, die in der Wertkonverter mit einer Funktion Formatierung, z. B. **Wprintf** oder **Swprintf**.

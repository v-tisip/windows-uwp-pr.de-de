...
<span data-ttu-id="d3d3a-110">Apps, die nicht mehr reagieren oder abstürzen, können zu einem Datenverlust und zu einer Beeinträchtigung des Benutzererlebnisses führen.</span><span class="sxs-lookup"><span data-stu-id="d3d3a-110">Apps that stop responding or crash can cause the user to lose data and have a poor experience.</span></span>
...
### <a name="corrective-action"></a><span data-ttu-id="d3d3a-165">Maßnahmen</span><span class="sxs-lookup"><span data-stu-id="d3d3a-165">Corrective action</span></span>
...
### <a name="test-details"></a><span data-ttu-id="d3d3a-191">Testdetails</span><span class="sxs-lookup"><span data-stu-id="d3d3a-191">Test details</span></span>
...
### <a name="span-idbinscope-1spanallowpartiallytrustedcallersattribute"></a><span data-ttu-id="d3d3a-208"><span id="binscope-1"></span>AllowPartiallyTrustedCallersAttribute</span><span class="sxs-lookup"><span data-stu-id="d3d3a-208"><span id="binscope-1"></span>AllowPartiallyTrustedCallersAttribute</span></span>
...
**<span data-ttu-id="d3d3a-234">Anmerkungen</span><span class="sxs-lookup"><span data-stu-id="d3d3a-234">Remarks</span></span>**
...
<span data-ttu-id="d3d3a-247">Dieser Test wird nur für Apps ausgeführt, die in nicht verwalteten Sprachen geschrieben wurden, z.B. mit C oder C++.</span><span class="sxs-lookup"><span data-stu-id="d3d3a-247">This test is performed only on apps written in unmanaged languages, such as by using C or C++.</span></span>
...
<span data-ttu-id="d3d3a-258">**Fehlermeldung des Zertifizierungskits für Windows-Apps:** AppContainerCheck Test failed (Fehler beim AppContainerCheck-Test).</span><span class="sxs-lookup"><span data-stu-id="d3d3a-258">**Windows App Certification Kit error message:** AppContainerCheck Test failed.</span></span>
...
**<span data-ttu-id="d3d3a-270">Was ist zu tun, wenn die App den Test nicht besteht?</span><span class="sxs-lookup"><span data-stu-id="d3d3a-270">What to do if your app fails this test</span></span>**
...
<span data-ttu-id="d3d3a-285">*PAGE\-SIZE* ist der Standardwert von *SectionAlignment* für ausführbare Dateien.</span><span class="sxs-lookup"><span data-stu-id="d3d3a-285">*PAGE\-SIZE* is the default *SectionAlignment* for executables.</span></span>
...
<span data-ttu-id="d3d3a-297">Apps müssen die APIs für UWP-apps (Windows-Runtime- oder unterstützte Win32-APIs), um für den Microsoft Store zertifiziert zu werden verwenden.</span><span class="sxs-lookup"><span data-stu-id="d3d3a-297">Apps must use the APIs for UWP apps (Windows Runtime or supported Win32 APIs) to be certified for the Microsoft Store.</span></span> <span data-ttu-id="d3d3a-298">Dieser Test ermittelt auch Situationen, in denen eine verwaltete Binärdatei eine Abhängigkeit von einer Funktion außerhalb des genehmigten Profils aufweist.</span><span class="sxs-lookup"><span data-stu-id="d3d3a-298">This test also identifies situations where a managed binary takes a dependency on a function outside of the approved profile.</span></span>
...
## <a name="performance-tests"></a><span data-ttu-id="d3d3a-308">Leistungstests</span><span class="sxs-lookup"><span data-stu-id="d3d3a-308">Performance tests</span></span>
...
<span data-ttu-id="d3d3a-319">Wenn bei diesem Test Fehler ermittelt werden, sollten Sie bei der Behandlung dieses Problems folgende Schritte berücksichtigen:</span><span class="sxs-lookup"><span data-stu-id="d3d3a-319">If this test fails, consider the following when addressing the issue:</span></span>
...
<p><span data-ttu-id="d3d3a-349">In der tatsächlichen Meldung enthält "{Bildname}“ den Namen des fehlenden Bilds.</span><span class="sxs-lookup"><span data-stu-id="d3d3a-349">In the actual message, {image name} contains the name of the image that is missing.</span></span></p>
</td></tr>
<tr><td>
...
### <a name="test-details"></a><span data-ttu-id="d3d3a-411">Testdetails</span><span class="sxs-lookup"><span data-stu-id="d3d3a-411">Test details</span></span>
...
-   <span data-ttu-id="d3d3a-416">Stellen Sie sicher, dass die App nicht über Links zu Debugversionen eines Frameworks verfügt und dass die Erstellung mit einer Releaseversion erfolgt.</span><span class="sxs-lookup"><span data-stu-id="d3d3a-416">Make sure the app isn't linking to debug versions of a framework and that it is building with a release version.</span></span> <span data-ttu-id="d3d3a-417">Wenn diese App .NET-Komponenten enthält, sollten Sie sich vergewissern, dass Sie die richtige Version des .NET-Frameworks installiert haben.</span><span class="sxs-lookup"><span data-stu-id="d3d3a-417">If this app contains .NET components, make sure that you have installed the correct version of the .NET framework.</span></span>

## <a name="file-encoding-test"></a><span data-ttu-id="d3d3a-418">Test der Dateicodierung</span><span class="sxs-lookup"><span data-stu-id="d3d3a-418">File encoding test</span></span>

### <a name="utf-8-file-encoding"></a><span data-ttu-id="d3d3a-419">UTF-8-Dateicodierung</span><span class="sxs-lookup"><span data-stu-id="d3d3a-419">UTF-8 file encoding</span></span>

### <a name="background"></a><span data-ttu-id="d3d3a-420">Hintergrund</span><span class="sxs-lookup"><span data-stu-id="d3d3a-420">Background</span></span>

<span data-ttu-id="d3d3a-421">HTML-, CSS- und JavaScript-Dateien müssen im UTF-8-Format codiert sein und über eine entsprechende Bytereihenfolgemarke (BOM) verfügen, um von der Bytecode-Zwischenspeicherung profitieren und bestimmte Laufzeitfehlerbedingungen vermeiden zu können.</span><span class="sxs-lookup"><span data-stu-id="d3d3a-421">HTML, CSS, and JavaScript files must be encoded in UTF-8 form with a corresponding byte-order mark (BOM) to benefit from bytecode caching and avoid certain runtime error conditions.</span></span>

### <a name="test-details"></a><span data-ttu-id="d3d3a-422">Testdetails</span><span class="sxs-lookup"><span data-stu-id="d3d3a-422">Test details</span></span>

<span data-ttu-id="d3d3a-423">Testet den Inhalt der App-Pakete, um sicherzustellen, dass darin die richtige Dateicodierung verwendet wird.</span><span class="sxs-lookup"><span data-stu-id="d3d3a-423">Test the contents of app packages to make sure that they use the correct file encoding.</span></span>

### <a name="corrective-action"></a><span data-ttu-id="d3d3a-424">Maßnahmen</span><span class="sxs-lookup"><span data-stu-id="d3d3a-424">Corrective Action</span></span>

<span data-ttu-id="d3d3a-425">Öffnen Sie die betroffene Datei, und wählen Sie in Visual Studio im Menü **Datei** die Option **Speichern unter**.</span><span class="sxs-lookup"><span data-stu-id="d3d3a-425">Open the affected file and select **Save As** from the **File** menu in Visual Studio.</span></span> <span data-ttu-id="d3d3a-426">Wählen Sie neben der Schaltfläche **Speichern** das Dropdownsteuerelement aus, und wählen Sie **Mit Codierung speichern** aus.</span><span class="sxs-lookup"><span data-stu-id="d3d3a-426">Select the drop-down control next to the **Save** button and select **Save with Encoding**.</span></span> <span data-ttu-id="d3d3a-427">Wählen Sie im Dialogfeld mit den erweiterten Speicheroptionen die Option **Unicode (UTF-8 mit Signatur)** aus, und klicken Sie auf **OK**.</span><span class="sxs-lookup"><span data-stu-id="d3d3a-427">From the **Advanced** save options dialog, choose the Unicode (UTF-8 with signature) option and click **OK**.</span></span>

## <a name="direct3d-feature-level-test"></a><span data-ttu-id="d3d3a-428">Test der Direct3D-Featureebene</span><span class="sxs-lookup"><span data-stu-id="d3d3a-428">Direct3D feature level test</span></span>

### <a name="direct3d-feature-level-support"></a><span data-ttu-id="d3d3a-429">Test auf Unterstützung der Direct3D-Featureebene</span><span class="sxs-lookup"><span data-stu-id="d3d3a-429">Direct3D feature level support</span></span>

<span data-ttu-id="d3d3a-430">Testet MicrosoftDirect3D-Apps, um sicherzustellen, dass sie auf Geräten mit älterer Grafikhardware nicht abstürzen.</span><span class="sxs-lookup"><span data-stu-id="d3d3a-430">Tests Microsoft Direct3D apps to ensure that they won't crash on devices with older graphics hardware.</span></span>

### <a name="background"></a><span data-ttu-id="d3d3a-431">Hintergrund</span><span class="sxs-lookup"><span data-stu-id="d3d3a-431">Background</span></span>

<span data-ttu-id="d3d3a-432">Microsoft Store müssen alle Anwendungen mit Direct3D richtig gerendert bzw. ordnungsgemäß auf Feature Grafikkarten der Featureebene 9\-1-Karten.</span><span class="sxs-lookup"><span data-stu-id="d3d3a-432">Microsoft Store requires all applications using Direct3D to render properly or fail gracefully on feature level 9\-1 graphics cards.</span></span>

<span data-ttu-id="d3d3a-433">Da die Benutzer die Grafikhardware ihrer Geräte nach der Installation der App ändern können, muss Ihre App für den Fall, dass Sie eine Featureebene höher als 9\-1 verwenden, beim Start erkennen, ob die aktuelle Hardware die Mindestanforderungen erfüllt.</span><span class="sxs-lookup"><span data-stu-id="d3d3a-433">Because users can change the graphics hardware in their device after the app is installed, if you choose a minimum feature level higher than 9\-1, your app must detect at launch whether or not the current hardware meets the minimum requirements.</span></span> <span data-ttu-id="d3d3a-434">Wenn die Mindestanforderungen nicht erfüllt sind, muss Ihre App dem Benutzer eine Meldung mit den Direct3D-Anforderungen anzeigen.</span><span class="sxs-lookup"><span data-stu-id="d3d3a-434">If the minimum requirements are not met, the app must display a message to the user detailing the Direct3D requirements.</span></span> <span data-ttu-id="d3d3a-435">Wird zudem eine App auf ein Gerät heruntergeladen, mit dem sie nicht kompatibel ist, sollte sie dies beim Start erkennen und dem Kunden eine Meldung bezüglich der erforderlichen Voraussetzungen anzeigen.</span><span class="sxs-lookup"><span data-stu-id="d3d3a-435">Also, if an app is downloaded on a device with which it is not compatible, it should detect that at launch and display a message to the customer detailing the requirements.</span></span>

### <a name="test-details"></a><span data-ttu-id="d3d3a-436">Testdetails</span><span class="sxs-lookup"><span data-stu-id="d3d3a-436">Test Details</span></span>

<span data-ttu-id="d3d3a-437">Bei diesem Test wird überprüft, ob Apps unter der Featureebene9\-1 richtig gerendert werden.</span><span class="sxs-lookup"><span data-stu-id="d3d3a-437">The test will validate if the apps render accurately on feature level 9\-1.</span></span>

### <a name="corrective-action"></a><span data-ttu-id="d3d3a-438">Maßnahmen</span><span class="sxs-lookup"><span data-stu-id="d3d3a-438">Corrective Action</span></span>

<span data-ttu-id="d3d3a-439">Stellen Sie sicher, dass die App unter der Direct3D-Featureebene9\-1 richtig gerendert wird. Dies gilt auch, wenn die App für die Ausführung auf einer höheren Featureebene bestimmt ist.</span><span class="sxs-lookup"><span data-stu-id="d3d3a-439">Ensure that your app renders correctly on Direct3D feature level 9\-1, even if you expect it to run at a higher feature level.</span></span> <span data-ttu-id="d3d3a-440">Weitere Informationen finden Sie unter [Entwickeln für unterschiedliche Direct3D-Featureebenen](http://go.microsoft.com/fwlink/p/?LinkID=253575).</span><span class="sxs-lookup"><span data-stu-id="d3d3a-440">See [Developing for different Direct3D feature levels](http://go.microsoft.com/fwlink/p/?LinkID=253575) for more info.</span></span>

### <a name="direct3d-trim-after-suspend"></a><span data-ttu-id="d3d3a-441">Direct3D-Kürzung nach dem Anhalten</span><span class="sxs-lookup"><span data-stu-id="d3d3a-441">Direct3D Trim after suspend</span></span>

> <span data-ttu-id="d3d3a-442">**Hinweis:**  Dieser Test gilt nur für UWP-apps für Windows 8.1 und höher entwickelt.</span><span class="sxs-lookup"><span data-stu-id="d3d3a-442">**Note**  This test only applies to UWP apps developed for Windows 8.1 and later.</span></span>

### <a name="background"></a><span data-ttu-id="d3d3a-443">Hintergrund</span><span class="sxs-lookup"><span data-stu-id="d3d3a-443">Background</span></span>

<span data-ttu-id="d3d3a-444">Wenn von der App auf ihrem jeweiligen Direct3D-Gerät [**Trim**](https://msdn.microsoft.com/library/windows/desktop/Dn280346) nicht aufgerufen wird, gibt sie keinen Speicher frei, der für die vorherigen 3D-Arbeitsschritte zugeordnet wurde.</span><span class="sxs-lookup"><span data-stu-id="d3d3a-444">If the app does not call [**Trim**](https://msdn.microsoft.com/library/windows/desktop/Dn280346) on its Direct3D device, the app will not release memory allocated for its earlier 3D work.</span></span> <span data-ttu-id="d3d3a-445">Dies erhöht das Risiko, dass Apps aufgrund von unzureichendem Systemspeicher beendet werden.</span><span class="sxs-lookup"><span data-stu-id="d3d3a-445">This increases the risk of apps being terminated due to system memory pressure.</span></span>

### <a name="test-details"></a><span data-ttu-id="d3d3a-446">Testdetails</span><span class="sxs-lookup"><span data-stu-id="d3d3a-446">Test Details</span></span>

<span data-ttu-id="d3d3a-447">Apps werden auf die Einhaltung der d3d-Anforderungen überprüft. Außerdem wird sichergestellt, dass Apps beim Anhalterückruf (Suspend) eine neue [**Trim**](https://msdn.microsoft.com/library/windows/desktop/Dn280346)-API aufrufen.</span><span class="sxs-lookup"><span data-stu-id="d3d3a-447">Checks apps for compliance with d3d requirements and ensures that apps are calling a new [**Trim**](https://msdn.microsoft.com/library/windows/desktop/Dn280346) API upon their Suspend callback.</span></span>

### <a name="corrective-action"></a><span data-ttu-id="d3d3a-448">Maßnahmen</span><span class="sxs-lookup"><span data-stu-id="d3d3a-448">Corrective Action</span></span>

<span data-ttu-id="d3d3a-449">Von der App sollte die [**Trim**](https://msdn.microsoft.com/library/windows/desktop/Dn280346)-API für ihre [**IDXGIDevice3**](https://msdn.microsoft.com/library/windows/desktop/Dn280345)-Schnittstelle vor jedem Anhaltevorgang aufgerufen werden.</span><span class="sxs-lookup"><span data-stu-id="d3d3a-449">The app should call the [**Trim**](https://msdn.microsoft.com/library/windows/desktop/Dn280346) API on its [**IDXGIDevice3**](https://msdn.microsoft.com/library/windows/desktop/Dn280345) interface anytime it is about to be suspended.</span></span>

## <a name="app-capabilities-test"></a><span data-ttu-id="d3d3a-450">Test der App-Funktionen</span><span class="sxs-lookup"><span data-stu-id="d3d3a-450">App Capabilities test</span></span>

### <a name="special-use-capabilities"></a><span data-ttu-id="d3d3a-451">Sonderfunktionen</span><span class="sxs-lookup"><span data-stu-id="d3d3a-451">Special use capabilities</span></span>

### <a name="background"></a><span data-ttu-id="d3d3a-452">Hintergrund</span><span class="sxs-lookup"><span data-stu-id="d3d3a-452">Background</span></span>

<span data-ttu-id="d3d3a-453">Sonderfunktionen sind für sehr spezielle Szenarien vorgesehen.</span><span class="sxs-lookup"><span data-stu-id="d3d3a-453">Special use capabilities are intended for very specific scenarios.</span></span> <span data-ttu-id="d3d3a-454">Diese Funktionen dürfen nur mit Unternehmenskonten genutzt werden.</span><span class="sxs-lookup"><span data-stu-id="d3d3a-454">Only company accounts are allowed to use these capabilities.</span></span>

### <a name="test-details"></a><span data-ttu-id="d3d3a-455">Testdetails</span><span class="sxs-lookup"><span data-stu-id="d3d3a-455">Test Details</span></span>

<span data-ttu-id="d3d3a-456">Es wird überprüft, ob von der App die folgenden Funktionen deklariert werden:</span><span class="sxs-lookup"><span data-stu-id="d3d3a-456">Validate if the app is declaring any of the below capabilities:</span></span>

-   <span data-ttu-id="d3d3a-457">EnterpriseAuthentication</span><span class="sxs-lookup"><span data-stu-id="d3d3a-457">EnterpriseAuthentication</span></span>
-   <span data-ttu-id="d3d3a-458">SharedUserCertificates</span><span class="sxs-lookup"><span data-stu-id="d3d3a-458">SharedUserCertificates</span></span>
-   <span data-ttu-id="d3d3a-459">DocumentsLibrary</span><span class="sxs-lookup"><span data-stu-id="d3d3a-459">DocumentsLibrary</span></span>

<span data-ttu-id="d3d3a-460">Falls eine oder mehrere dieser Funktionen deklariert werden, wird Benutzern während des Tests eine Warnung angezeigt.</span><span class="sxs-lookup"><span data-stu-id="d3d3a-460">If any of these capabilities are declared, the test will display a warning to the user.</span></span>

### <a name="corrective-actions"></a><span data-ttu-id="d3d3a-461">Maßnahmen</span><span class="sxs-lookup"><span data-stu-id="d3d3a-461">Corrective Actions</span></span>

<span data-ttu-id="d3d3a-462">Sie können erwägen, die Sonderfunktion zu entfernen, wenn diese für die App nicht erforderlich ist.</span><span class="sxs-lookup"><span data-stu-id="d3d3a-462">Consider removing the special use capability if your app doesn't require it.</span></span> <span data-ttu-id="d3d3a-463">Die Verwendung dieser Funktionen unterliegt außerdem einer weiteren Prüfung anhand der Richtlinie für die Aufnahme.</span><span class="sxs-lookup"><span data-stu-id="d3d3a-463">Additionally, use of these capabilities are subject to additional on-boarding policy review.</span></span>

## <a name="windows-runtime-metadata-validation"></a><span data-ttu-id="d3d3a-464">Windows-Runtime-Metadatenvalidierung</span><span class="sxs-lookup"><span data-stu-id="d3d3a-464">Windows Runtime metadata validation</span></span>

### <a name="background"></a><span data-ttu-id="d3d3a-465">Hintergrund</span><span class="sxs-lookup"><span data-stu-id="d3d3a-465">Background</span></span>

<span data-ttu-id="d3d3a-466">Es wird sichergestellt, dass die Komponenten, die als Teil einer App mitgeliefert werden, mit dem UWP-Typsystem kompatibel sind.</span><span class="sxs-lookup"><span data-stu-id="d3d3a-466">Ensures that the components that ship in an app conform to the UWP type system.</span></span>

### <a name="test-details"></a><span data-ttu-id="d3d3a-467">Testdetails</span><span class="sxs-lookup"><span data-stu-id="d3d3a-467">Test Details</span></span>

<span data-ttu-id="d3d3a-468">Es wird überprüft, ob die **WINMD**-Dateien im Paket den UWP-Regeln entsprechen.</span><span class="sxs-lookup"><span data-stu-id="d3d3a-468">Verifies that the **.winmd** files in the package conform to UWP rules.</span></span>

### <a name="corrective-actions"></a><span data-ttu-id="d3d3a-469">Maßnahmen</span><span class="sxs-lookup"><span data-stu-id="d3d3a-469">Corrective Actions</span></span>

-   <span data-ttu-id="d3d3a-470">**ExclusiveTo-Attributtest:** Stellen Sie sicher, dass von UWP-Klassen keine Schnittstellen implementiert werden, die für eine andere Klasse als „ExclusiveTo” gekennzeichnet sind.</span><span class="sxs-lookup"><span data-stu-id="d3d3a-470">**ExclusiveTo attribute test:** Ensure that UWP classes don't implement interfaces that are marked as ExclusiveTo another class.</span></span>
-   <span data-ttu-id="d3d3a-471">**Test auf Anordnung von Typen:** Stellen Sie sicher, dass die Metadaten für alle UWP-Typen in der WINMD-Datei enthalten sind, die im App-Paket über den längsten Namen mit Namespaceübereinstimmung verfügt.</span><span class="sxs-lookup"><span data-stu-id="d3d3a-471">**Type location test:** Ensure that the metadata for all UWP types is located in the winmd file that has the longest namespace-matching name in the app package.</span></span>
-   <span data-ttu-id="d3d3a-472">**Test auf Groß-/Kleinschreibung von Typnamen:** Stellen Sie sicher, dass alle UWP-Typen im App-Paket eindeutige Namen aufweisen, bei denen die Groß-/Kleinschreibung nicht zu berücksichtigen ist.</span><span class="sxs-lookup"><span data-stu-id="d3d3a-472">**Type name case-sensitivity test:** Ensure that all UWP types have unique, case-insensitive names within your app package.</span></span> <span data-ttu-id="d3d3a-473">Vergewissern Sie sich außerdem, dass UWP-Typnamen im App-Paket nicht auch als Namespacename verwendet werden.</span><span class="sxs-lookup"><span data-stu-id="d3d3a-473">Also ensure that no UWP type name is also used as a namespace name within your app package.</span></span>
-   <span data-ttu-id="d3d3a-474">**Test auf Korrektheit des Typnamens:** Stellen Sie sicher, dass im globalen Namespace oder im Windows-Namespace der obersten Ebene keine UWP-Typen vorhanden sind.</span><span class="sxs-lookup"><span data-stu-id="d3d3a-474">**Type name correctness test:** Ensure there are no UWP types in the global namespace or in the Windows top-level namespace.</span></span>
-   <span data-ttu-id="d3d3a-475">**Test auf Korrektheit der allgemeinen Metadaten:** Stellen Sie sicher, dass der zum Generieren der Typen verwendete Compiler in Bezug auf die UWP-Spezifikationen auf dem neuesten Stand ist.</span><span class="sxs-lookup"><span data-stu-id="d3d3a-475">**General metadata correctness test:** Ensure that the compiler you are using to generate your types is up to date with the UWP specifications.</span></span>
-   <span data-ttu-id="d3d3a-476">**Eigenschaftentest:** Stellen Sie sicher, dass alle Eigenschaften einer UWP-Klasse über eine get-Methode verfügen (set-Methoden sind optional).</span><span class="sxs-lookup"><span data-stu-id="d3d3a-476">**Properties test:** ensure that all properties on a UWP class have a get method (set methods are optional).</span></span> <span data-ttu-id="d3d3a-477">Vergewissern Sie sich, dass der Typ des Rückgabewerts der get-Methode für alle Eigenschaften von UWP-Typen jeweils mit dem Typ des Eingabeparameters der set-Methode übereinstimmt.</span><span class="sxs-lookup"><span data-stu-id="d3d3a-477">Ensure that the type of the get method return value matches the type of the set method input parameter, for all properties on UWP types.</span></span>

## <a name="package-sanity-tests"></a><span data-ttu-id="d3d3a-478">Tests für die Paketintegrität</span><span class="sxs-lookup"><span data-stu-id="d3d3a-478">Package Sanity tests</span></span>

### <a name="platform-appropriate-files-test"></a><span data-ttu-id="d3d3a-479">Test auf für Plattform geeignete Dateien</span><span class="sxs-lookup"><span data-stu-id="d3d3a-479">Platform appropriate files test</span></span>

<span data-ttu-id="d3d3a-480">Apps, von denen gemischte Binärdateien installiert werden, stürzen je nach der Prozessorarchitektur von Benutzern möglicherweise ab oder werden nicht richtig ausgeführt.</span><span class="sxs-lookup"><span data-stu-id="d3d3a-480">Apps that install mixed binaries may crash or not run correctly depending upon the user’s processor architecture.</span></span>

### <a name="background"></a><span data-ttu-id="d3d3a-481">Hintergrund</span><span class="sxs-lookup"><span data-stu-id="d3d3a-481">Background</span></span>

<span data-ttu-id="d3d3a-482">Bei diesem Test werden die Binärdateien in einem App-Paket auf Architekturkonflikte geprüft.</span><span class="sxs-lookup"><span data-stu-id="d3d3a-482">This test validates the binaries in an app package for architecture conflicts.</span></span> <span data-ttu-id="d3d3a-483">Ein App-Paket sollte keine Binärdateien enthalten, die unter der im Manifest angegebenen Prozessorarchitektur nicht verwendet werden können.</span><span class="sxs-lookup"><span data-stu-id="d3d3a-483">An app package should not include binaries that can't be used on the processor architecture specified in the manifest.</span></span> <span data-ttu-id="d3d3a-484">Das Einfügen von nicht unterstützten Binärdateien kann zum Absturz der App oder zu einem unnötigen Anstieg der Paketgröße einer App führen.</span><span class="sxs-lookup"><span data-stu-id="d3d3a-484">Including unsupported binaries can lead to your app crashing or an unnecessary increase in the app package size.</span></span>

### <a name="test-details"></a><span data-ttu-id="d3d3a-485">Testdetails</span><span class="sxs-lookup"><span data-stu-id="d3d3a-485">Test Details</span></span>

<span data-ttu-id="d3d3a-486">Es wird überprüft, ob die Bitanzahl jeder einzelnen Datei im PE-Header korrekt ist, wenn Querverweise mit der Prozessorarchitekturdeklaration des App-Pakets eingerichtet werden.</span><span class="sxs-lookup"><span data-stu-id="d3d3a-486">Validates that each file's "bitness" in the PE header is appropriate when cross-referenced with the app package processor architecture declaration</span></span>

### <a name="corrective-action"></a><span data-ttu-id="d3d3a-487">Maßnahmen</span><span class="sxs-lookup"><span data-stu-id="d3d3a-487">Corrective Action</span></span>

<span data-ttu-id="d3d3a-488">Befolgen Sie diese Richtlinien, um sicherzustellen, dass Ihr App-Paket nur Dateien enthält, die von der im App-Manifest angegebenen Architektur unterstützt werden:</span><span class="sxs-lookup"><span data-stu-id="d3d3a-488">Follow these guidelines to ensure that your app package only contains files supported by the architecture specified in the app manifest:</span></span>

-   <span data-ttu-id="d3d3a-489">Wenn die Zielprozessorarchitektur der App den Prozessortyp "Neutral" aufweist, kann die App keine x86-, x64- oder ARM-Binärdateien oder -Abbilddateien enthalten.</span><span class="sxs-lookup"><span data-stu-id="d3d3a-489">If the Target Processor Architecture for your app is Neutral processor Type, the app package cannot contain x86, x64, or ARM binary or image type files.</span></span>

-   <span data-ttu-id="d3d3a-490">Wenn die Zielprozessorarchitektur der App den Prozessortyp "x86" aufweist, darf das App-Paket nur x86-Binärdateien oder -Abbilddateien enthalten.</span><span class="sxs-lookup"><span data-stu-id="d3d3a-490">If the Target Processor Architecture for your app is x86 processor type, the app package must only contain x86 binary or image type files.</span></span> <span data-ttu-id="d3d3a-491">Wenn das Paket x64- oder ARM-Binärtypen oder -Imagetypen enthält, tritt beim Test ein Fehler auf.</span><span class="sxs-lookup"><span data-stu-id="d3d3a-491">If the package contains x64 or ARM binary or image types, it will fail the test.</span></span>

-   <span data-ttu-id="d3d3a-492">Wenn die Zielprozessorarchitektur der App den Prozessortyp "x64" aufweist, muss das App-Paket x64-Binärdateien oder -Abbilddateien enthalten.</span><span class="sxs-lookup"><span data-stu-id="d3d3a-492">If the Target Processor Architecture for your app is x64 processor type, the app package must contain x64 binary or image type files.</span></span> <span data-ttu-id="d3d3a-493">Beachten Sie, dass das Paket in diesem Fall auch x86-Dateien enthalten kann. Für die Hauptoberfläche der App sollte jedoch die x64-Binärdatei genutzt werden.</span><span class="sxs-lookup"><span data-stu-id="d3d3a-493">Note that in this case the package can also include x86 files, but the primary app experience should utilize the x64 binary.</span></span>

    <span data-ttu-id="d3d3a-494">Falls das Paket jedoch ARM-Binärdateien oder -Abbilddateien oder ausschließlich x86-Binärdateien oder -Abbilddateien enthält, tritt beim Test ein Fehler auf.</span><span class="sxs-lookup"><span data-stu-id="d3d3a-494">However, if the package contains ARM binary or image type files, or only contains x86 binaries or image type files, it will fail the test.</span></span>

-   <span data-ttu-id="d3d3a-495">Wenn die Zielprozessorarchitektur der App den Prozessortyp "ARM" aufweist, darf das App-Paket nur ARM-Binärdateien oder -Abbilddateien enthalten.</span><span class="sxs-lookup"><span data-stu-id="d3d3a-495">If the Target Processor Architecture for your app is ARM processor type, the app package must only contain ARM binary or image type files.</span></span> <span data-ttu-id="d3d3a-496">Wenn das Paket x64- oder x86-Binärdateien oder -Abbilddateien enthält, tritt beim Test ein Fehler auf.</span><span class="sxs-lookup"><span data-stu-id="d3d3a-496">If the package contains x64 or x86 binary or image type files, it will fail the test.</span></span>

### <a name="supported-directory-structure-test"></a><span data-ttu-id="d3d3a-497">Test der unterstützten Verzeichnisstruktur</span><span class="sxs-lookup"><span data-stu-id="d3d3a-497">Supported Directory Structure test</span></span>

<span data-ttu-id="d3d3a-498">Es wird sichergestellt, dass von Anwendungen während der Installation keine Unterverzeichnisse erstellt werden, die länger als MAX\-PATH sind.</span><span class="sxs-lookup"><span data-stu-id="d3d3a-498">Validates that applications are not creating subdirectories as part of installation that are longer than MAX\-PATH.</span></span>

### <a name="background"></a><span data-ttu-id="d3d3a-499">Hintergrund</span><span class="sxs-lookup"><span data-stu-id="d3d3a-499">Background</span></span>

<span data-ttu-id="d3d3a-500">Komponenten des Betriebssystems (z.B. Trident, WWAHost usw.) sind für Dateisystempfade intern auf den MAX\-PATH-Wert begrenzt und funktionieren nicht ordnungsgemäß, wenn längere Pfade verwendet werden.</span><span class="sxs-lookup"><span data-stu-id="d3d3a-500">OS components (including Trident, WWAHost, etc.) are internally limited to MAX\-PATH for file system paths and will not work correctly for longer paths.</span></span>

### <a name="test-details"></a><span data-ttu-id="d3d3a-501">Testdetails</span><span class="sxs-lookup"><span data-stu-id="d3d3a-501">Test Details</span></span>

<span data-ttu-id="d3d3a-502">Es wird überprüft, ob für Pfade im Installationsverzeichnis der App der MAX\-PATH-Wert überschritten wird.</span><span class="sxs-lookup"><span data-stu-id="d3d3a-502">Verifies that no path within the app install directory exceeds MAX\-PATH.</span></span>

### <a name="corrective-action"></a><span data-ttu-id="d3d3a-503">Maßnahmen</span><span class="sxs-lookup"><span data-stu-id="d3d3a-503">Corrective Action</span></span>

<span data-ttu-id="d3d3a-504">Verwenden Sie eine kürzere Verzeichnisstruktur bzw. kürzere Dateinamen.</span><span class="sxs-lookup"><span data-stu-id="d3d3a-504">Use a shorter directory structure, and or file name.</span></span>

## <a name="resource-usage-test"></a><span data-ttu-id="d3d3a-505">Test für die Ressourcennutzung</span><span class="sxs-lookup"><span data-stu-id="d3d3a-505">Resource Usage test</span></span>

### <a name="winjs-background-task-test"></a><span data-ttu-id="d3d3a-506">Test der WinJS-Hintergrundaufgabe</span><span class="sxs-lookup"><span data-stu-id="d3d3a-506">WinJS Background Task test</span></span>

<span data-ttu-id="d3d3a-507">Mit dem Test der WinJS-Hintergrundaufgabe wird sichergestellt, dass JavaScript-Apps über die richtigen close-Anweisungen verfügen, damit die Apps nicht unnötig Akkuenergie verbrauchen.</span><span class="sxs-lookup"><span data-stu-id="d3d3a-507">WinJS background task test ensures that JavaScript apps have the proper close statements so apps don’t consume battery.</span></span>

### <a name="background"></a><span data-ttu-id="d3d3a-508">Hintergrund</span><span class="sxs-lookup"><span data-stu-id="d3d3a-508">Background</span></span>

<span data-ttu-id="d3d3a-509">Apps mit JavaScript-Hintergrundaufgaben müssen "Close()" als letzte Anweisung der Hintergrundaufgabe aufrufen.</span><span class="sxs-lookup"><span data-stu-id="d3d3a-509">Apps that have JavaScript background tasks need to call Close() as the last statement in their background task.</span></span> <span data-ttu-id="d3d3a-510">Ist dies bei Apps nicht der Fall, kann das System unter Umständen nicht in den verbundenen Standbymodus zurückkehren, was zu einer schnelleren Entleerung des Akkus führen kann.</span><span class="sxs-lookup"><span data-stu-id="d3d3a-510">Apps that do not do this could keep the system from returning to connected standby mode and result in draining the battery.</span></span>

### <a name="test-details"></a><span data-ttu-id="d3d3a-511">Testdetails</span><span class="sxs-lookup"><span data-stu-id="d3d3a-511">Test Details</span></span>

<span data-ttu-id="d3d3a-512">Wenn für Apps im Manifest keine Hintergrundaufgabe angegeben ist, gilt der Test als bestanden.</span><span class="sxs-lookup"><span data-stu-id="d3d3a-512">If the app does not have a background task file specified in the manifest, the test will pass.</span></span> <span data-ttu-id="d3d3a-513">Andernfalls wird beim Testen die JavaScript-Hintergrundaufgabendatei analysiert, die im App-Paket angegeben ist, und nach einer Close()-Anweisung gesucht.</span><span class="sxs-lookup"><span data-stu-id="d3d3a-513">Otherwise the test will parse the JavaScript background task file that is specified in the app package, and look for a Close() statement.</span></span> <span data-ttu-id="d3d3a-514">Wird diese gefunden, gilt der Test als bestanden. Falls nicht, gilt der Test als nicht bestanden.</span><span class="sxs-lookup"><span data-stu-id="d3d3a-514">If found, the test will pass; otherwise the test will fail.</span></span>

### <a name="corrective-action"></a><span data-ttu-id="d3d3a-515">Maßnahmen</span><span class="sxs-lookup"><span data-stu-id="d3d3a-515">Corrective Action</span></span>

<span data-ttu-id="d3d3a-516">Aktualisieren Sie den JavaScript-Hintergrundcode so, dass „Close()” richtig aufgerufen wird.</span><span class="sxs-lookup"><span data-stu-id="d3d3a-516">Update the background JavaScript code to call Close() correctly.</span></span>


## <a name="related-topics"></a><span data-ttu-id="d3d3a-517">Verwandte Themen</span><span class="sxs-lookup"><span data-stu-id="d3d3a-517">Related topics</span></span>

* [<span data-ttu-id="d3d3a-518">Tests für Windows Desktop-Brücke-Apps</span><span class="sxs-lookup"><span data-stu-id="d3d3a-518">Windows Desktop Bridge app tests</span></span>](windows-desktop-bridge-app-tests.md)
* [<span data-ttu-id="d3d3a-519">Microsoft Store-Richtlinien</span><span class="sxs-lookup"><span data-stu-id="d3d3a-519">Microsoft Store Policies</span></span>](https://msdn.microsoft.com/library/windows/apps/Dn764944)

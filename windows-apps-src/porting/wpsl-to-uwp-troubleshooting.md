...
Wenn Ihre App beendet wird und Sie nur wissen, dass während der XAML-Markupanalyse ein Ausnahmefehler ausgelöst wurde, ist die Ursache möglicherweise ein Verweis auf eine fehlende Ressource (d.h. eine Ressource, deren Schlüssel für WindowsPhone Silverlight-Apps, jedoch nicht für Windows10-Apps vorhanden ist, z.B. einige System-**TextBlock**-Stilschlüssel). Es kann sich auch um eine Ausnahme handeln, die innerhalb eines **UserControl**-Elements, eines benutzerdefinierten Steuerelements oder eines benutzerdefinierten Layoutpanels ausgelöst wurde.

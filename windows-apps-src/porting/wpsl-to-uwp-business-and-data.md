...
Mit Launchern und Choosern (zu finden im Namespace **Microsoft.Phone.Tasks** ), kann eine WindowsPhone Silverlight-app interagieren, mit dem Betriebssystem, um allgemeine Vorgänge wie Verfassen einer e-Mails, Auswählen eines Fotos oder Teilen bestimmter Arten von Daten mit einer anderen app. Suchen Sie nach **Microsoft.Phone.Tasks** , unter dem Thema [Windows Phone Silverlight zu Windows 10 Namespace- und klassenzuordnungen](wpsl-to-uwp-namespace-and-class-mappings.md) zu den entsprechenden UWP-Typ finden. Diese reichen von ähnlichen Mechanismen (so genannten Launchern und Pickern) bis zur Implementierung eines Vertrags zum Teilen von Daten zwischen Apps.

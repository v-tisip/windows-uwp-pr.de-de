...
## <a name="identify-packaged-application-compatibility-issues"></a>Identifizieren von verpackten Probleme mit der Anwendungskompatibilität
...
In diesem Problem ist die Anwendung fehl, um einer log-Datei in der Paketpfad zu schreiben. Dies würde vorschlagen, eine Datei Umleitung Korrektur hilfreich ist.
...
Achten Sie darauf, dass die Community Beiträge unserer [GitHub](https://github.com/Microsoft/MSIX-PackageSupportFramework/tree/develop) -Seite zu überprüfen. Es ist möglich, dass andere Entwickler eine ähnlich wie Ihr Problem gelöst haben und ein Laufzeit-Update freigegeben haben.
...
Dies erhalten etwas Sie, die wie folgt aussieht.
...
Erforderliche 32-Bit- und 64-Bit-PSF-DLLs und ausführbaren Dateien in das Paketverzeichnis hinzufügen. Orientieren Sie sich an der folgenden Tabelle. Sie sollten auch alle Runtime Fixes enthalten, die Sie benötigen. In unserem Beispiel benötigen wir der Datei Umleitung Runtime beheben.
...
Der Inhalt sollte nun etwa wie folgt aussehen.
...
| applications | ausführbare | Den relativen Pfad der ausführbaren Datei, die Sie starten möchten. In den meisten Fällen können Sie diesen Wert aus der Paketmanifestdatei abrufen, bevor Sie sie ändern. Es ist der Wert der `Executable` -Attribut des der `Application` Element. |
...
| Prozesse | ausführbare | Dies ist in den meisten Fällen ist der Name des, die `executable` oben mit den Pfad und den Dateinamen Erweiterung konfiguriert. |
| Korrekturen | DLL-Datei | Relativen Pfad zu der Reparatur,.msix/.appx zu laden. |
| Korrekturen | config | (Optional) Steuert, wie die Korrektur Verteilerliste verhält. Das genaue Format dieses Werts variiert Korrektur durch Korrektur Basis wie jeder Korrektur dieses "Blob" interpretiert werden kann, wie möglich. |
...
Sie können Visual Studio verwenden, um ein Laufzeit-Update Debuggen, erweitern ein Laufzeit-Update oder ein neues erstellen. Sie müssen diese Vorgänge erfolgreich ausgeführt werden kann.

> [!div class="checklist"]
> * Hinzufügen eines verpackungsprojekts
> * Projekt für das Update Runtime hinzufügen
...
| Projekt | Zweck |
|-------|-----------|
| DesktopApplicationPackage | Dieses Projekt basiert auf dem [Windows Application Packaging-Projekt](desktop-to-uwp-packaging-dot-net.md) , und es gibt die des MSIX-Pakets. |
...
### <a name="create-a-package-solution"></a>Erstellen einer Flight-Lösung
...
Im **Projektmappen-Explorer**mit der rechten Maustaste in des Verpacken-Projekts, **Bearbeiten**auswählen und dann am unteren Rand der Projektdatei hinzufügen:
...
### <a name="add-project-for-the-runtime-fix"></a>Projekt für das Update Runtime hinzufügen
...
Wenn Sie Debuggen oder einen vorhandenen Runtime Fix erweitern möchten, fügen Sie die Runtime-Update-Dateien, die Sie abgerufen, indem Sie die Anleitung im Abschnitt [Suchen nach einer Lösung für die Laufzeit](#find) dieses Handbuchs.
...
Hinzufügen der **PSF** Nuget-Paket dieses Projekt mit der gleichen Anleitung im vorherigen Abschnitt beschrieben.
...
| Korrekturen | config | (Optional) Steuert, wie die Korrektur DLL verhält. Das genaue Format dieses Werts variiert Korrektur durch Korrektur Basis wie jeder Korrektur dieses "Blob" interpretiert werden kann, wie möglich. |
...
>[!NOTE]
> Obwohl Visual Studio Ihnen die einfachste Entwicklung bietet und Debuggen, es einige Einschränkungen gibt, später in diesem Handbuch erläutern wir, andere debugging Techniken, die Sie anwenden können.
...
Deklarieren Sie die ``FIXUP_DEFINE_EXPORTS`` Makro und fügen eine Include-Anweisung für die `fixup_framework.h` am Anfang jeder. CPP-Datei für die Funktionen Ihrer Laufzeit-Updates hinzufügen möchten.
...
Beispielsweise können Sie erzeugen, eine Funktion Ersatz für die `CreateFile` Funktion. Die Implementierung aufrufen kann die `CopyFile` -Funktion, aber die Implementierung von der `CopyFile` Funktionsaufruf möglicherweise die `CreateFile` Funktion. Dies kann dazu führen, dass einer unendlichen rekursiven Zyklus der Aufrufe an die `CreateFile` Funktion.

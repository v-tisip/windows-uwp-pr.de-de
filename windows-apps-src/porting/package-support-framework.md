...
---
# <a name="apply-runtime-fixes-to-an-msix-package-by-using-the-package-support-framework"></a>Anwenden von Runtime-Updates auf ein MSIX-Paket mit dem Paket-Framework-Unterstützung
...
### <a name="using-process-monitor-to-identify-an-issue"></a>Verwenden zum Identifizieren der ein Problem Prozess überwachen

[Prozess-Monitor](https://docs.microsoft.com/en-us/sysinternals/downloads/procmon) ist ein leistungsfähiges Dienstprogramm ermöglicht Ihnen, eine app-Datei und Registrierungsvorgänge und die Ergebnisse.  Dies hilft Ihnen, Probleme mit der Anwendungskompatibilität zu verstehen.  Nach dem Öffnen Prozess überwachen, fügen Sie einen Filter (Filter > Filter...) nur Ereignisse für die Anwendung ausführbare enthalten.
...
In der ersten Ausgabe, die in diesem Bild angezeigt wird, wird die Anwendung nicht aus der Datei "Config.txt" zu lesen, die im Pfad "C:\Windows\SysWOW64" befindet. Es ist unwahrscheinlich, dass die Anwendung versucht, diesen Pfad direkt referenzieren. In den meisten Fällen versucht, mit der ein relativer Pfad aus der Datei gelesen und in der Standardeinstellung ist "System32/SysWOW64" Arbeitsverzeichnis der Anwendung. Dies zeigt, dass die Anwendung die aktuelle Arbeitsverzeichnis, das an einer beliebigen Stelle im Paket festgelegt werden, um erwartet wird. Suchen in der Appx, sehen Sie, dass die Datei in demselben Verzeichnis wie die ausführbare Datei vorhanden ist.
...
Die Anwendung nicht in dieser Ausgabe eine log-Datei in ihrem Paketpfad zu schreiben. Dies würde wird empfohlen, eine Datei Umleitung Korrektur hilfreich ist.

<a id="find" />

## <a name="find-a-runtime-fix"></a>Suchen nach einer Runtime-Lösung

Die PSF enthält die Common Language Runtime-Updates, die Sie jetzt, wie z. B. die Datei Umleitung Korrektur verwenden können.

### <a name="file-redirection-fixup"></a>Konvertieren der Datei-Umleitung

Die [Datei Umleitung konvertieren](https://github.com/Microsoft/MSIX-PackageSupportFramework/tree/master/fixups/FileRedirectionFixup) können umgeleitet Versuche zum Schreiben oder Lesen von Daten in einem Verzeichnis, das von einer Anwendung ist, die in einem MSIX-Container ausgeführt wird.
...
Achten Sie darauf, dass Sie die Community Beiträge unserer [GitHub](https://github.com/Microsoft/MSIX-PackageSupportFramework) -Seite zu überprüfen. Es ist möglich, dass andere Entwickler eine ähnlich wie Ihr Problem gelöst haben und ein Laufzeit-Update freigegeben haben.
...
Wenn Sie eine Datei .msix (oder AppX) zu haben, können Sie die Paketordner und Dateien von Grund auf neu erstellen.
...
In Visual Studio mit der rechten Maustaste in Ihrer Lösung oder Projektknoten, und wählen Sie einen der Befehle Nuget-Pakete verwalten.  Suchen Sie nach **Microsoft.PackageSupportFramework** oder **PSF** , das Paket "NuGet.org" gefunden. Anschließend können installieren Sie es.
...
Erforderlichen 32-Bit- und 64-Bit-PSF-DLLs und ausführbaren Dateien in das Paketverzeichnis hinzufügen. Orientieren Sie sich an der folgenden Tabelle. Sie sollten auch alle Common Language Runtime-Updates enthalten, die Sie benötigen. In unserem Beispiel benötigen wir die Datei Umleitung-Runtime-Problembehandlung.
...
| Prozesse | ausführbare | In den meisten Fällen, dies ist der Name des, die `executable` oben mit der Erweiterung Pfad und konfiguriert. |
| Korrekturen | DLL-Datei | Relativen Pfad zu der Korrektur.msix/.appx geladen. |
| Korrekturen | config | (Optional) Steuert das Verhalten der Verteilerliste konvertieren. Die genaue Format dieses Werts ist Konvertieren von-Korrektur zu, wie jeder Korrektur dieses "Blob" interpretiert werden kann, wie möglich. |

Die `applications`, `processes`, und `fixups` Schlüssel sind Arrays. Das bedeutet, dass Sie die Datei config.json verwenden können, mehr als eine Anwendung, Prozess und Korrektur DLL-Datei an.
...
>[!NOTE]
> Denken Sie daran, zuerst das Paket deinstalliert werden soll.
...
Ein alternatives Verfahren für die Diagnose von Kompatibilitätsproblemen verpackten Anwendung ist die Verwendung der Trace konvertieren. Diese DLL-Datei ist die PSF enthalten und bietet eine detaillierte Ansicht der app Verhalten, ähnlich wie Process Monitor Diagnosedaten.  Es ist speziell auf Probleme mit der Anwendungskompatibilität anzuzeigen.  Verwenden Sie die Korrektur Trace, die DLL für das Paket hinzufügen Ihrer config.json die folgenden Fragment-Komponente hinzufügen und dann Verpacken und Installieren Ihrer Anwendung.
...
Standardmäßig werden die Korrektur Trace Fehler, die berücksichtigt werden möglicherweise "erwartet".  Beispielsweise Versuch Anwendungen bedingungslos Löschen einer Datei ohne zu überprüfen, um festzustellen, ob sie vorhanden ist, wird das Ergebnis ignoriert. Dies hat die unerwünschten Folgen, die einige unerwartete Fehler, gefiltert erhalten möglicherweise so melden Sie sich im obigen Beispiel wir an, um alle Fehler von Dateisystem-Funktionen zu empfangen. Wir tun dies, da wir von vor, die das Lesen aus der Datei Config.txt fehlschlägt wissen mit "die Meldung Datei nicht gefunden". Dies ist ein Fehler, der häufig ermittelt und nicht in der Regel davon ausgegangen, dass unerwartet ist. In der Praxis ist es wahrscheinlich am besten, beginnen Sie mit dem Filterung nur für unerwartete Fehler und dann zurückgreifen auf alle Fehler liegt ein Problem, das noch nicht identifiziert werden kann.
...
Sie können Visual Studio verwenden, ein Update-Runtime-Debuggen, erweitern ein Laufzeit-Update oder von Grund auf neu erstellen. Sie müssen für diese Vorgänge erfolgreich ausgeführt werden kann.

> [!div class="checklist"]
> * Hinzufügen eines verpackungsprojekts
> * Fügen Sie Projekt für die Common Language Runtime-Problembehandlung
> * Fügen Sie ein Projekt, die PSF-Startprogramms gestartet wird.
...
| Projekt | Zweck |
|-------|-----------|
| DesktopApplicationPackage | Dieses Projekt basiert auf dem [Windows Application Packaging-Projekt](desktop-to-uwp-packaging-dot-net.md) , und es gibt die MSIX-Paket. |
| Runtimefix | Dies ist eine Bibliothek Dynamic-Linked-Projekt, die eine oder mehrere Ersatzfunktionen enthält, die als die Common Language Runtime-Problembehandlung dienen. |
| PSFLauncher | Dies ist eine leere C++-Projekt. Dies ist ein Ort zum Abrufen der Runtime verteilbaren Dateien des Pakets Support-Frameworks. Es gibt eine ausführbare Datei. Die ausführbare Datei ist der erste Schritt, der ausgeführt wird, wenn Sie die Projektmappe zu starten. |
...
Weitere Informationen zu Windows Application Packaging Project finden Sie im [Paket Ihrer Anwendung mithilfe von Visual Studio](desktop-to-uwp-packaging-dot-net.md).
...
Wenn Sie Debuggen oder eine vorhandene Runtime Korrektur zu erweitern möchten, fügen Sie die Runtime-Update-Dateien, die Sie abgerufen, indem Sie die Anleitung im Abschnitt [Suchen nach einer Lösung für die Laufzeit](#find) dieses Handbuchs.

Wenn Sie beabsichtigen, ein völlig neues Update zu erstellen, fügen Sie nicht nichts auf dieses Projekt nur noch hinzu. Wir helfen Ihnen, dieses Projekt weiter unten in diesem Handbuch die richtigen Dateien hinzugefügt. Für den Moment fahren wir Einrichten Ihrer Projektmappe fort.

### <a name="add-a-project-that-starts-the-psf-launcher-executable"></a>Fügen Sie ein Projekt, die PSF-Startprogramms gestartet wird.

Fügen Sie der Projektmappe ein **Leeres Projekt** für C++-Projekt hinzu.

![Leeres Projekt](images/desktop-to-uwp/blank-app.png)

Fügen Sie das **PSF** Nuget-Paket für dieses Projekt mit der gleichen Anleitung im vorherigen Abschnitt beschrieben.

Öffnen der Eigenschaftenseiten für das Projekt, und klicken Sie auf der Seite **Allgemein** Einstellungen legen Sie die **Ziel-Name** -Eigenschaft auf ``PSFLauncher32`` oder ``PSFLauncher64`` je nach der Architektur der Anwendung.
...
Wählen Sie die PSF Startprogramm Projekt und Ihre desktop-Anwendung, und wählen Sie dann die Schaltfläche " **OK** ".

![Desktopprojekt](images/desktop-to-uwp/package-project-references.png)

>[!NOTE]
> Wenn Sie den Quellcode für Ihre Anwendung besitzen, wählen Sie einfach das PSF-Start-Projekt. Wir zeigen Ihnen wie die EXE-Datei zu verweisen, wenn Sie eine Konfigurationsdatei erstellen.
...
Fügen Sie eine Datei mit dem Namen ``config.json`` dem Verpacken-Projekt, klicken Sie dann, kopieren und fügen Sie die folgenden Json-Text in die Datei. Legen Sie die **Aktion Paket** -Eigenschaft auf **Inhalte**.
...
Geben Sie einen Wert für jeden Schlüssel. Verwenden Sie in der folgenden Tabelle als Anleitung.
...
| Korrekturen | config | (Optional) Steuert, wie die Korrektur DLL verhält. Die genaue Format dieses Werts ist Konvertieren von-Korrektur zu, wie jeder Korrektur dieses "Blob" interpretiert werden kann, wie möglich. |
...
Drücken Sie in Visual Studio F5, um den Debugger zu starten.  Müssen Sie zunächst, die gestartet wird, ist der startanwendung PSF, die wiederum die Ziel-desktop-Anwendung gestartet wird.  Um die Ziel-desktop-Anwendung zu debuggen, müssen Sie manuell den Prozess der desktop-Anwendung zuordnen, indem Sie **Debuggen**auswählen->**an den Prozess anhängen**, und wählen Sie dann den Anwendungsprozess. Um das Debuggen einer .NET-Anwendung mit ein Update native-Runtime-DLL zu ermöglichen, wählen Sie verwalteten und systemeigenen Code (im gemischten Modus Debuggen).  

Nachdem Sie dies eingerichtet haben, können Sie Haltepunkte neben Codezeilen in der desktop-Anwendungscode und die Update-Runtime-Projekts festlegen. Wenn Sie den Quellcode für Ihre Anwendung besitzen, werden Sie Haltepunkte nur neben Codezeilen in Ihrem Projekt der Runtime Update festlegen.
...
Wenn es keine noch eine Laufzeit für das Problem beheben, aufgelöst, können Sie erstellen ein neues-Runtime-Update Schreiben des Ersatzfunktionen und einschließlich aller Konfigurationsdaten ist, die sinnvoll. Sehen wir uns auf jedem Teil.

### <a name="replacement-functions"></a>Ersatzfunktionen

Identifizieren Sie zunächst, welche Funktion ruft fehl, wenn Ihre Anwendung in einem MSIX-Container ausgeführt wird. Anschließend können Sie den Austausch Funktionen erstellen, die Sie mit den Laufzeit-Manager stattdessen aufrufen möchten. Dies bietet Ihnen die Möglichkeit, die Implementierung einer Funktion mit Verhalten zu ersetzen, die die Regeln der modernen-Runtime-Umgebung entspricht.

Öffnen Sie das Projekt der Runtime-beheben, das Sie weiter oben in diesem Handbuch erstellt haben, in Visual Studio.
...
>[!IMPORTANT]
>Stellen Sie sicher, dass die `FIXUP_DEFINE_EXPORTS` Makro angezeigt wird, bevor Sie die Include-Anweisung.
...
#### <a name="protect-against-recursive-calls-to-functions-in-runtime-fixes"></a>Schutz vor rekursiven-Aufrufe von Funktionen in der Runtime-Updates

Sie können optional anwenden, die `reentrancy_guard` geben, um Ihre Funktionen, die Schutz gegen rekursiv-Aufrufe von Funktionen in der Runtime-Updates.
...
Wenn Visual Studio die einfachste Entwicklung und das debugging Erfahrung Ihnen bietet, gibt es einige Einschränkungen.

Zunächst Debuggens mit F5 führt die Anwendung durch die Bereitstellung von losen Dateien aus dem Paket Layout Ordnerpfad, installieren, sondern von einer .msix / AppX-Pakets.  Der Layoutordner verfügt in der Regel nicht dieselben sicherheitseinschränkungen, die als einen installiere Paket-Ordner. Daher kann es nicht Paket Pfad Zugriff Denial-Fehler vor der Anwendung ein Laufzeit-Updates reproduziert werden.
...
Ein weiteres Problem mit Visual Studio ist, dass es keine integrierten Unterstützung für die Verbindung vom Debugger gestartet untergeordnete Prozesse enthält.   Dies erschwert es zum Debuggen von Logik im Startpfad der Zielanwendung, die nach dem Start von Visual Studio manuell angefügt werden muss.
...
(ausgeführt wird, bis die Anwendung beginnt und unterbricht)

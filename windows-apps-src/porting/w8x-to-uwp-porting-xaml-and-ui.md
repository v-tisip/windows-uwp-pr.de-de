...
Sie müssen Ihren Code ändern, die mit Charms integriert, aber Sie müssen einige Elemente der Benutzeroberfläche Ihrer app, die von der Leiste Charms stattfinden, die nicht Teil der Windows 10-Shell ist hinzufügen. Universellen 8.1-Apps unter Windows 10 verfügt über eine eigene Ersatz vom System gerendert Chrome in der Titelleiste der app bereitgestellten Benutzeroberfläche.

## <a name="controls-and-control-styles-and-templates"></a>Steuerelemente und Steuerelementstile/-vorlagen

Eine universelle 8.1-app, die auf Windows 10 ausgeführt wird das 8.1 aussehen und Verhalten in Bezug auf die Steuerelemente beibehalten. Wenn Sie diese app in eine Windows 10-app portieren, sind aber einige Unterschiede beim Aussehen und Verhalten zu beachten. Die Architektur und das Design von Steuerelementen ist im Wesentlichen unverändert für Windows 10-apps, damit die Änderungen vor allem um [Design-Sprache](#design-language-in-windows-10), Vereinfachung und Verbesserung der benutzerfreundlichkeit sind.

**Hinweis:**  der PointerOver-Ansichtszustand ist in benutzerdefinierten Stile/Vorlagen in Windows 10-apps und in Windows-Runtime 8.x-apps, jedoch nicht in Windows Phone Store-apps relevant. Aus diesem Grund (und aufgrund der Systemressourcenschlüssel, die für Windows 10-apps unterstützt werden), wird empfohlen, dass Sie erneut die benutzerdefinierten Stile/Vorlagen aus Ihrer Windows-Runtime 8.x-apps verwenden, wenn Sie Ihre app auf Windows 10 portieren.
Wenn Sie, dass möchten Ihre benutzerdefinierten Stile/Vorlagen die aktuelle Gruppe der Ansichtszustände, und sind gelten vorgenommene leistungsverbesserungen für den standardmäßigen Stilen/Vorlagen, und klicken Sie dann eine Kopie der neuen Windows 10-Standardvorlage bearbeiten und erneut anwenden der Anpassungen darauf. Ein Beispiel für eine Leistungsverbesserung besteht darin, dass alle **Border**-Elemente, die ein **ContentPresenter**- Element oder Panel-Element bisher eingeschlossen haben, entfernt wurden. Der Rahmen wird jetzt von einem untergeordneten Element gerendert.
...
| **AppBar**, [**CommandBar**](https://msdn.microsoft.com/library/windows/apps/hh701927) | In einer Windows 10-app haben **App-Leiste** und [**Befehlsleiste**](https://msdn.microsoft.com/library/windows/apps/hh701927) eine Schaltfläche **finden Sie unter mehr** (Auslassungspunkte). |
...
| [**CommandBar**](https://msdn.microsoft.com/library/windows/apps/hh701927) | Bei einer Windows Phone Store-App hat der Wert von [**CommandBar.IsSticky**](https://msdn.microsoft.com/library/windows/apps/hh701944) keinen Einfluss darauf, ob die Leiste einfach ausgeblendet werden kann. Für eine Windows 10 ignoriert app, wenn **IsSticky** auf "true", und klicken Sie dann auf die **CommandBar** festgelegt ist eine Geste zum einfachen ausblenden. |
...
| Verschiedenes | Akzentfarbe gilt für eine Windows Phone Store-apps und Windows 10-apps, aber nicht für Windows-Runtime 8.x-apps.  |
...
Sie können erneut verwenden werden XAML-Markup aus einer Windows-Runtime 8.x-app, in denen literalen Dimensionswerten im Markup (z. B. um Größe von Formen oder anderen Elementen (z. B. Typografie) verwendet. In einigen Fällen ein größerer Skalierungsfaktor für eine universelle 8.1-app auf einem Gerät für eine Windows 10-app als verwendet (z. B. 150 % wird verwendet, wobei 140 % wurde vor und 200 % anstatt 180 % verwendet). Ja, wenn Sie feststellen, dass diese Literalwerte jetzt auf Windows 10 zu groß sind, versuchen Sie es dann mit dem Wert 0,8 multiplizieren. Weitere Informationen finden Sie unter [Reaktionsfähiges Design 101 – Grundlagen für UWP-Apps](https://msdn.microsoft.com/library/windows/apps/dn958435).
...
In einer Windows-Runtime 8.x-app Wenn die app deaktiviert und erneut aktiviert wird, (falls sie angezeigt wurde), wird die verkleinerte Ansicht geschlossen und stattdessen die vergrößerte Ansicht angezeigt. In einer Windows Phone Store-app und in einer Windows 10-app bleibt die verkleinerte Ansicht angezeigt, wenn sie angezeigt wurde.

In einer Windows Phone Store-app und in einer Windows 10-app wird die verkleinerte Ansicht geschlossen, wenn die zurück-Schaltfläche gedrückt wird. Für eine Windows-Runtime 8.x-app ist keine integrierte Schaltfläche "zurück" Verarbeitung vorhanden, sodass die Frage nicht stellt.

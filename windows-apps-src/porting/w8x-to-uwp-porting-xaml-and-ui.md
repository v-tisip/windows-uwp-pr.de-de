...
Sie müssen Ihren Code ändern, die mit Charms integriert, aber Sie müssen einige Elemente der Benutzeroberfläche Ihrer app, die von der Leiste Charms stattfinden, die nicht Teil der Windows 10-Shell ist hinzufügen. Eine universellen 8.1-app auf Windows 10 verfügt über eine eigene Ersatz vom System gerendert Chrome in der app-Titelleiste bereitgestellten Benutzeroberfläche.
...
| [**TextBox (Textfeld)**](https://msdn.microsoft.com/library/windows/apps/br209683) | Die Standardschriftgröße für ein [**TextBox**](https://msdn.microsoft.com/library/windows/apps/br209683)-Element hat sich von11 in15 geändert. |
...
Sie können erneut verwenden XAML-Markup aus einer Windows-Runtime 8.x-app, in denen literalen Dimensionswerten im Markup (z. B. zur Größe von Formen oder andere Elemente, z. B. Typografie) verwendet. In einigen Fällen ein größerer Skalierungsfaktor auf einem Gerät für eine Windows 10-app als für eine universelle 8.1-app verwendet wird (z. B. 150 % wird verwendet, wobei 140 % wurde vor dem und 200 % anstatt 180 % verwendet). Ja, wenn Sie feststellen, dass diese Literalwerte jetzt in Windows 10 zu groß sind, versuchen Sie es dann mit dem Wert 0,8 multiplizieren. Weitere Informationen finden Sie unter [Reaktionsfähiges Design 101 – Grundlagen für UWP-Apps](https://msdn.microsoft.com/library/windows/apps/dn958435).
...
Hier sind die Aspekte der [**GridView**](https://msdn.microsoft.com/library/windows/apps/br242705) und [**ListView**](https://msdn.microsoft.com/library/windows/apps/br242878) , die geändert oder werden nicht in Windows 10 unterstützt.

-   Die [**IsSwipeEnabled**](https://msdn.microsoft.com/library/windows/apps/hh702518) -Eigenschaft (Windows-Runtime 8.x-apps nur) ist für Windows 10-apps nicht unterstützt. Die API ist noch vorhanden, ihre Verwendung hat aber keine Auswirkung. Mit Ausnahme von der Wischbewegung nach unten (wird nicht unterstützt, da sie den Daten zufolge nicht gefunden wird) und dem Rechtsklick (ist für die Anzeige eines Kontextmenüs reserviert) werden alle bisherigen Auswahlbewegungen unterstützt.
...
In einer Windows-Runtime 8.x-app Wenn die app deaktiviert und erneut aktiviert ist, (falls sie angezeigt wurde), wird die verkleinerte Ansicht geschlossen und stattdessen die vergrößerte Ansicht angezeigt. In einer Windows Phone Store-app und in einer Windows 10-app wird die verkleinerte Ansicht weiter angezeigt, sofern sie wurde.

In einer Windows Phone Store-app und in einer Windows 10-app wird die verkleinerte Ansicht geschlossen, wenn die zurück-Schaltfläche gedrückt wird. Für eine Windows-Runtime 8.x-app ist es keine integrierte Schaltfläche "zurück" Verarbeitung, sodass sich diese Frage nicht.

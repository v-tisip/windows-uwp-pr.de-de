...
Hier erfahren Sie, wie Sie eine App für die Universelle Windows-Plattform (UWP) erstellen, die einen Dienst für andere UWP-Apps bereitstellen kann, und wie Sie diesen Dienst nutzen.
...
-   Wenn Sie gefragt werden, wählen Sie eine **Zielversion** für das Projekt auszuwählen, wählen Sie mindestens **10.0.14393** aus. Wenn Sie das neue `SupportsMultipleInstances` -Attribut verwenden möchten, müssen Sie Visual Studio2017 verwenden und als Ziel **10.0.15063** (**Windows 10 Creator's Update**) oder höher auswählen.
...
2.  Fügen Sie im Projekt „AppServiceProvider” einen Projekt-zu-Projekt-Verweis auf das neue Projekt „MyAppService” hinzu. Dieser Schritt ist unerlässlich, da der App-Dienst ohne das Hinzufügen der Referenz keine Laufzeit damit verbindet, auch wenn Sie keine Kompilierungsfehler sehen.
...
Es wird eine Verzögerung implementiert, damit der Dienst **async**-Methoden im OnRequestReceived-Handler verwenden kann. Dadurch wird sichergestellt, dass der Aufruf von **OnRequestReceived** erst abgeschlossen wird, wenn die Nachricht verarbeitet wurde.  [**SendResponseAsync**](https://msdn.microsoft.com/library/windows/apps/dn921722) wird verwendet, um beim Abschluss eine Antwort zu senden. **SendResponseAsync** signalisiert nicht den Abschluss des Aufrufs. [**SendMessageAsync**](https://msdn.microsoft.com/library/windows/apps/dn921712) wird durch den Abschluss der Verzögerung signalisiert, dass **OnRequestReceived** abgeschlossen wurde. Es bietet sich an, **SendMessageAsync()** in einem Versuch/Final-Block aufzurufen, da für die Verzögerung **Complete()** aufgerufen werden muss, auch wenn **SendMessageAsync()** eine Ausnahme auslöst.
...
- Vergewissern Sie sich, dass der Eintrag `<Extensions>` und sein untergeordnetes Elemente zur Datei "Package.appxmanifest" des App-Dienstanbieterprojekts wie oben angegeben hinzugefügt wurden [Hinzufügen einer App-Diensterweiterung zu „package.appxmanifest”](#add-an-app-service-extension-to-package-appxmanifest).
...
- Stellen Sie sicher, dass `AppServiceConnection.PackageFamilyName` dem Paketfamiliennamen der App-Dienstanbieterkomponente wie in [Hinzufügen eine App-Diensterweiterung zu "Package.appxmanifest"](#add-an-app-service-extension-to-package-appxmanifest) angegeben entspricht.

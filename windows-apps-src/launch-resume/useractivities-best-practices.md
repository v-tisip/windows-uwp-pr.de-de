...
* **Erstellen Sie Aktivitäten nicht für Aktionen, die von anderen Benutzern abgeschlossen.** Wenn ein externes Konto des Benutzers eine Nachricht sendet oder @-mentions sie innerhalb Ihrer app sollte nicht erstellen Sie eine Aktivität für diese. Diese Art von Aktion wird vom Info-Center-Benachrichtigungen besser bereitgestellt.
  * Szenarien für die Zusammenarbeit sind eine Ausnahme: Wenn mehrere Benutzer auf die gleiche Aktivität (z. B. einem Word-Dokument) zusammen arbeiten, es werden Fälle, in denen ein anderer Benutzer nach dem Benutzer bearbeitet hat. In diesem Fall empfiehlt es sich, aktualisieren Sie die vorhandene Aktivität, um die Änderungen zu aktualisieren, die auf das Dokument vorgenommen wurden. Dies würde umfassen, aktualisieren die vorhandenen Inhalten Benutzeraktivität-Daten ohne ein neues Verlauf Element erstellen.
...
* **Document-basierte apps** – Erstellen einer Aktivität pro Dokument, das mit mindestens ein Verlauf Elemente reflektieren nutzen. Es ist wichtig, die Aktivität zu aktualisieren, sobald das Dokument Änderungen vorgenommen wurden.
* **Spiele** – eine Aktivität für jeden Spiel speichern oder Welt zu erstellen. Wenn Ihr Spiel nur eine einzige Sequenz von Ebenen unterstützt, können Sie die gleiche Aktivität im Laufe der Zeit erneut veröffentlichen, obwohl Sie möglicherweise die Content-Daten zum Anzeigen der aktuelle Status oder Erfolge aktualisieren möchten.
...
* **Media-Wiedergabe-apps** – Erstellen einer Aktivität pro logische Gruppierung von Inhalt (z. B. eine Wiedergabeliste, Programm oder eigenständigen Inhalt). Die zugrunde liegende Frage für app-Entwickler ist ein jedes Inhaltselement (TV-Folge, Titel) gibt an, ob als eigenständiges Inhalt oder als Teil einer Sammlung zählt. Wenn der Benutzer entscheidet, um eine Sammlung oder sequenziellen Inhalte wiederzugeben, ist die Sammlung als Ganzes als in der Regel die Aktivität. Wenn sie entscheiden, um einen einzelnen Teilinhalt wiederzugeben, ist dieser eine Teilinhalt die Aktivität. Finden Sie unter folgenden spezifischere Richtlinien.
  ...
  * **Film** – ein Films ist eine einzelne Inhaltselement und sollte eine eigene Verlaufsdatensatz. Wenn der Benutzer Überwachen der Film Teil-Wege über beendet, ist es wünschenswert, notieren Sie ihre Position. Wenn sie es in der Zukunft fortsetzen möchten, könnte die Aktivität Fortsetzen des Films, wo sie haben aufgehört, oder bitten Sie den Benutzer auch verwenden, um fortsetzen oder am Anfang starten.

## <a name="user-activity-design"></a>Benutzer Aktivität design

Benutzeraktivitäten bestehen aus drei Komponenten: einem URI-Aktivierung visual-Daten und Content-Metadaten.
...
Benutzeraktivitäten können auch Content-Metadaten enthalten, die Windows und Cortana verwenden, um Aktivitäten kategorisieren und Rückschlüsse zu generieren. Aktivitäten können dann gruppiert werden, um ein bestimmtes Thema, z. B. eine Position (wenn der Benutzer Ferienreisen untersucht wird), Objekt (wenn der Benutzer eine Aktion untersucht ist) oder eine Aktion, (wenn der Benutzer für ein bestimmtes Produkt auf verschiedenen apps und Websites einkaufen ist). Es ist sinnvoll, die Substantiven und eine Aktivität beteiligt Verben darstellen. 

Im folgenden Beispiel wird die Content-Metadaten JSON, folgen den Standards der [Schema.org](https://schema.org/), steht für das Szenario: "John verärgert Tiere mit gespielt Steve."
...
* [Benutzeraktivitäten (Projekt "ROME" Dokumente)](https://docs.microsoft.com/windows/project-rome/user-activities/)

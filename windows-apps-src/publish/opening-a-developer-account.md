...
> [!NOTE]
> In einigen Fällen können die Bildschirme und Felder, die bei der Registrierung für ein Entwicklerkonto angezeigt werden, geringfügig von der folgenden Beschreibung abweichen. Die grundlegenden Informationen und Vorgehensweise sind jedoch gleich.

1.  Wechseln Sie zur [Registrierungsseite](http://go.microsoft.com/fwlink/p/?LinkId=615100), und klicken Sie auf **Registrieren**.
2.  Wenn Sie noch nicht mit einem Microsoft-Konto angemeldet sind, melden Sie sich jetzt an, oder erstellen Sie ein neues Microsoft-Konto. Das Microsoft-Konto, das Sie hier verwenden, ist das Konto, das Sie zum Anmelden bei Ihrem Entwicklerkonto verwenden.
3.  Wählen Sie [das Land/die Region](account-types-locations-and-fees.md#developer-account-and-app-submission-markets) aus, in dem bzw. in der Sie ansässig sind oder in dem bzw. in der sich Ihr Unternehmen befindet. Dies kann später nicht mehr geändert werden.
4.  Wählen Sie den gewünschten [Entwicklerkontotyp](account-types-locations-and-fees.md) (Einzelkonto oder Unternehmenskonto) aus. Dies kann später nicht mehr geändert werden. Wählen Sie also unbedingt den richtigen Kontotyp aus.
5.  Geben Sie den **Anzeigenamen des Herausgebers** an, den Sie verwenden möchten (maximal 50Zeichen). Wählen Sie den Namen mit Bedacht, da Kunden diesen Namen sehen und Ihre Apps unter diesem Namen kennenlernen. Für Unternehmenskonten müssen Sie den eingetragenen Unternehmens- oder Handelsnamen Ihrer Organisation verwenden. Wenn Sie einen Namen eingeben, der bereits von einem anderen Benutzer ausgewählt wurde oder dessen Nutzungsrechte bei einer anderen Person liegen, können Sie diesen Namen nicht verwenden. 

   > [!NOTE]
   > Stellen Sie sicher, dass sie zum Verwenden des hier eingegebenen Namens berechtigt sind. Wurde der von Ihnen gewählte Name bereits als Marke eingetragen oder urheberrechtlich geschützt, wird Ihr Konto unter Umständen geschlossen. Weitere Informationen finden Sie unter [Vereinbarung für App-Entwickler](https://docs.microsoft.com/legal/windows/agreements/app-developer-agreement). Wenn eine andere Person einen Herausgeberanzeigenamen verwendet, den Sie markenrechtlich oder anderweitig geschützt haben, [wenden Sie sich an Microsoft](http://go.microsoft.com/fwlink/p/?LinkId=233777).    
...
10.  Klicken Sie auf **Fertig stellen**, um die Registrierung zu bestätigen. Ihre Zahlung verarbeitet werden, und es wird eine Meldung zur Bestätigung an Ihre e-Mail-Adresse senden.
...
> [!IMPORTANT]
> Damit um mehrere Benutzer auf Ihr Konto Developer Center zugreifen zu können, sollten mithilfe von Azure Active Directory Zuweisen von Rollen zu einzelnen Benutzern (statt sharing Zugriff auf das Microsoft-Konto). Jeder Benutzer kann so auf das Dev Center-Konto zugreifen, indem er sich mit seinen individuellen Azure AD-Anmeldeinformationen anmeldet. Weitere Informationen finden Sie unter [Verwalten von Kontobenutzern](manage-account-users.md).

Wenn Sie ein Firmenkonto erstellen, können diese Richtlinien hilfreich sein, wenn mehr als einer Person auf das Konto zugreifen, indem Sie signieren mit Microsoft-Konto, das das Konto geöffnet (nicht als einzelne Benutzer für das Konto Developer Center hinzugefügt).
...
-   Richten Sie eine e-Mail-Verteilerliste, die alle Benutzer umfasst, die die Entwicklerkonto zugreifen möchte, und fügen Sie diese e-Mail-Adresse für den [Sicherheitsinformationen, die das Microsoft-Konto zugeordneten [(https://account.microsoft.com/security). Dadurch werden alle Mitarbeiter in der Liste auf die Sicherheitscodes an diesen Aliasnamen gesendet wird. Wenn einrichten eine Verteilerliste ist nicht möglich, Sie können Ihre Sicherheitsinformationen e-Mail-Adresse eines einzelnen Benutzers hinzufügen, aber der Besitzer der e-Mail-Adresse werden als einziger zugreifen und Freigeben von den Sicherheitscode bei entsprechender Aufforderung (beispielsweise wenn neue Sicherheitsinformationen t hinzugefügt wird er berücksichtigt werden, oder wenn sie über ein neues Gerät erfolgt).
-   Fügen Sie eine Telefonnummer Sicherheitsinformationen für das Microsoft-Konto hinzu. Verwenden Sie eine Zahl, die eine Erweiterung ist nicht erforderlich und wird für wichtige Teammitglieder zugänglich sind.
-   Im Allgemeinen müssen Sie Entwickler [vertrauenswürdigen Geräten](https://support.microsoft.com/help/12369/microsoft-account-add-a-trusted-device) verwenden Ihres Unternehmens Entwicklerkonto anzumelden. Alle wichtigen Teammitglieder müssen auf diese vertrauenswürdigen Geräte zugreifen können. So müssen beim Zugriff auf das Konto weniger Sicherheitscodes gesendet werden. Die Anzahl von Codes ist begrenzt, die pro Konto und Woche generiert werden können.
...
-   Überprüfen Sie regelmäßig unter https://account.microsoft.com/security, ob Ihre Sicherheitsinformationen aktuell sind.


## <a name="microsoft-account-security"></a>Microsoft-Konto-Sicherheit
...
Es müssen mindestens zwei E-Mail-Adressen und/oder Telefonnummern für das Microsoft-Konto vorhanden sein. Wir empfehlen, so viele wie möglich hinzuzufügen. Beachten Sie bitte, dass einige Sicherheitsinformationen bestätigt werden müssen, ehe sie verwendet werden können. Überprüfen Sie außerdem regelmäßig, ob Ihre Sicherheitsinformationen aktuell sind. Sie können Ihre Sicherheitsinformationen verwalten, indem Sie zu https://account.microsoft.com/security wechseln und sich mit Ihrem Microsoft-Konto anmelden. Weitere Informationen finden Sie unter [Security Info & Sicherheitscodes](https://support.microsoft.com/help/12428/microsoft-account-security-info-and-security-codes) .

Wenn Sie zum Windows-Entwicklungscenter Dashboard mit Ihrem Microsoft-Konto anmelden, möglicherweise vom System aufgefordert, Sie Ihre Identität überprüfen durch Senden einen Sicherheitscode, den Sie angeben müssen, um den Vorgang abzuschließen. Es wird empfohlen, PCs, mit denen Sie häufig als *Vertrauenswürdige Geräte*festlegen. Beim Anmelden von einem vertrauenswürdigen Gerät werden nicht Sie in der Regel für einen Code aufgefordert, obwohl Sie möglicherweise gelegentlich in bestimmten Situationen aufgefordert werden, oder wenn Sie auf diesem Gerät in einen längeren Zeitraum noch nicht angemeldet. Weitere Informationen finden Sie unter [Hinzufügen eines vertrauenswürdigen Geräts mit Ihrem Microsoft-Konto](https://support.microsoft.com/help/12369/microsoft-account-add-a-trusted-device) .

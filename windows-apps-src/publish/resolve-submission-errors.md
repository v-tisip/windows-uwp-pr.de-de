...
Wenn Sie eine UWP-App einreichen, wird während der Vorverarbeitung möglicherweise ein Fehler angezeigt, wenn die Paketdatei keine von Visual Studio für den Store generierte „.appxupload“-Datei ist. Stellen Sie sicher, dass Sie führen Sie die Schritte in [einer UWP-app mit Visual Studio-Paket](../packaging/packaging-uwp-apps.md) , beim Erstellen Ihrer app-Paket-Datei, und nur der .appxupload-Datei auf der Seite [Pakete](upload-app-packages.md) der Übermittlung, keines Appx oder .appxbundle hochladen.
...
Wenn Sie planen, ein Paket zu senden, die Win32 und UWP Binärdateien enthält, stellen Sie sicher, dass Sie das Paket mithilfe der Windows-Packprojekt, die in Visual Studio 2017 Update 4 verfügbar ist, erstellen. Wenn Sie das Paket mit einer UWP-Projektvorlage erstellen, können Sie möglicherweise nicht übermitteln, die die Store oder Sideload es auf anderen PCs packen. Auch wenn das Paket erfolgreich veröffentlicht, möglicherweise die Verhalten in ungewöhnlich auf dem PC des Benutzers. Weitere Informationen finden Sie unter [Paket einer app mithilfe von Visual Studio (Desktop Bridge)]( https://docs.microsoft.com/windows/uwp/porting/desktop-to-uwp-packaging-dot-net).
...
> [!IMPORTANT]
>  Wenn der Name, den Sie verwenden möchten, nicht verfügbar ist, eine andere app möglicherweise bereits diesem Namen reserviert. Wenn Ihre app bereits unter diesem Namen veröffentlicht wird, oder wenn Sie meinen, Sie haben das Recht zu [Kontaktieren des Supports](https://go.microsoft.com/fwlink/p/?LinkId=331509)verwenden.  

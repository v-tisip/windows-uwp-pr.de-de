...
Bedenken Sie, dass alle Partner Center-Benutzer (einschließlich von Gruppen und Azure AD-Apps) ein aktives Konto in [Azure AD-Mandanten, der Ihr Partner Center-Konto zugeordnet ist,](associate-azure-ad-with-partner-center.md)vorhanden sein müssen. Die Benutzerverwaltung erfolgt pro Mandant. Sie müssen sich mit einem Managerkonto auf dem Mandanten anmelden, wenn Sie Benutzer hinzufügen oder bearbeiten möchten. Erstellen eines neuen Benutzers im Partner Center wird ebenfalls erstellt ein Konto für diesen Benutzer in Azure AD-Mandanten mit dem Sie angemeldet sind, und wirken sich diese Änderungen auf den Namen eines Benutzers im Partner Center wird die gleiche Änderungen in Azure AD-Mandanten Ihrer Organisation.

> [!NOTE]
> Wenn Ihre Organisation [Verzeichnisintegration](https://go.microsoft.com/fwlink/p/?LinkID=724033) zum Synchronisieren des lokalen Verzeichnisdienst mit Ihrem Azure AD verwendet, nicht Sie neue Benutzer, Gruppen oder Azure AD-Apps in Partner Center erstellen. Sie (oder ein anderer Administrator in Ihrem lokalen Verzeichnis) müssen sie direkt im lokalen Verzeichnis erstellen, bevor Sie können werden, finden Sie unter und diese im Partner Center hinzufügen.
...
Sie können eine Gruppe aus dem Verzeichnis der Organisation für Ihr Partner Center-Konto hinzufügen. Daraufhin kann jeder Benutzer, der Mitglied dieser Gruppe ist, mit den Berechtigungen der dieser Gruppe zugewiesenen Rolle darauf zugreifen.
...
2.  Wählen Sie in der angezeigten Liste eine oder mehrere Gruppen aus. Im Suchfeld können Sie nach bestimmten Gruppen suchen.
    > [!TIP]
    > Wenn Sie mehr als eine Gruppe hinzufügen zu Ihrem Partner Center-Konto auswählen, müssen Sie diesen die gleiche Rolle oder Gruppe von benutzerdefinierten Berechtigungen zuweisen. Wiederholen Sie zum Hinzufügen mehrerer Gruppen mit anderen Rollenberechtigungen die unten beschriebenen Schrittefür alle Rollen oder benutzerdefinierte Berechtigungen.

3.  Wenn Sie die Gruppen ausgewählt haben, klicken Sie auf **Ausgewählte hinzufügen**.
4.  Geben Sie im Abschnitt **Rollen** an, welche [Rollen oder angepassten Berechtigungen](set-custom-permissions-for-account-users.md) Sie für die ausgewählten Gruppen wünschen. Alle Mitglieder der Gruppe kann auf Ihr Partner Center-Konto mit den Berechtigungen, dass Sie auf die Gruppe, unabhängig von den Rollen/Berechtigungen mit individuellen Konto anwenden.
5.  Klicken Sie auf **Speichern**.


### <a name="create-a-new-group-account-in-your-organizations-directory-and-add-it-to-your-partner-center-account"></a>Erstellen eines neuen Gruppenkontos in das Verzeichnis Ihrer Organisation und Hinzufügen zu Ihrem Partner Center-Konto
...
4.  Geben Sie die [Rollen oder angepasste Berechtigungen](set-custom-permissions-for-account-users.md) für die Gruppe an. Alle Mitglieder der Gruppe kann auf Ihr Partner Center-Konto mit den Berechtigungen, dass Sie auf die Gruppe, unabhängig von den Rollen/Berechtigungen mit individuellen Konto anwenden.
...
### <a name="create-a-new-azure-ad-application-account-in-your-organizations-directory-and-add-it-to-your-partner-center-account"></a>Erstellen Sie ein neues Azure AD-Anwendung-Kontos im Verzeichnis Ihrer Organisation und für Ihr Partner Center-Konto hinzufügen
...
> [!IMPORTANT]
> Änderungen an der [Rolle oder Berechtigung](set-custom-permissions-for-account-users.md) wirken sich nur auf Partner Center-Zugriff aus. Alle anderen Änderungen (z. B. das ändern den Namen eines Benutzers oder der Gruppenmitgliedschaft oder der Antwort-URL und App-ID-URI für eine Azure AD-Anwendung) werden in Ihrer Organisation Azure AD-Mandanten sowie im Partner Center-Konto widergespiegelt. 

1.  Wählen Sie die Seite " **Benutzer** " (unter **kontoeinstellungen**) den Namen des Benutzer-, Gruppen oder Azure AD-Anwendungskontos, die Sie bearbeiten möchten.
...
3.  Änderungen, die im Zusammenhang mit Zugriff auf Partner Center aktivieren Sie oder deaktivieren Sie die Rolle(n), die Sie anwenden möchten, oder wählen Sie **Berechtigungen anpassen** und nehmen Sie die gewünschten Änderungen. Diese Änderungen nur wirken auf Partner Center zugreifen und die Berechtigungen in Azure AD-Mandanten Ihrer Organisation wird nicht geändert.
3.  Klicken Sie auf **Speichern**.

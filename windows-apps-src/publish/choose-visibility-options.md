...
- Alle Rezensionen Ihrer Tester werden im [Bericht "Rezensionen"](reviews-report.md) angezeigt. Allerdings sind diese Rezensionen nicht im App Store-Eintrag sichtbar, auch nachdem Ihre Übermittlung als **öffentliche Zielgruppe** festgelegt wird.  
- Wenn Sie die App von **private Zielgruppe** auf **öffentliche Zielgruppe** ändern, ist das angezeigte **Veröffentlichungsdatum** im Store-Eintrag das Datum der ersten Veröffentlichung an die öffentliche Zielgruppe (es sei denn, Sie geben ein anderes [Veröffentlichungsdatum](set-app-pricing-and-availability.md#display-release-date) an).

...
> [!NOTE]
> <span data-ttu-id="38077-105">In diesem Thema erhalten Sie Unterstützung bei der Verwaltung Ihrer C++/CX-Anwendung.</span><span class="sxs-lookup"><span data-stu-id="38077-105">This topic exists to help you maintain your C++/CX application.</span></span> <span data-ttu-id="38077-106">Es wird jedoch empfohlen, dass Sie [C++/WinRT](../cpp-and-winrt-apis/intro-to-using-cpp-with-winrt.md) für neue Anwendungen nutzen.</span><span class="sxs-lookup"><span data-stu-id="38077-106">But we recommend that you use [C++/WinRT](../cpp-and-winrt-apis/intro-to-using-cpp-with-winrt.md) for new applications.</span></span> <span data-ttu-id="38077-107">C++/WinRT ist eine vollständig standardisierte, moderne C++17 Sprachprojektion für Windows-Runtime-(WinRT)-APIs, die als headerdateibasierte Bibliothek implementiert ist und Ihnen einen erstklassigen Zugriff auf die moderne Windows-API bietet.</span><span class="sxs-lookup"><span data-stu-id="38077-107">C++/WinRT is an entirely standard modern C++17 language projection for Windows Runtime (WinRT) APIs, implemented as a header-file-based library, and designed to provide you with first-class access to the modern Windows API.</span></span> <span data-ttu-id="38077-108">Informationen zum Erstellen einer Komponente für Windows-Runtime mit C++ / WinRT, finden Sie unter [Erstellen von Ereignissen in C++ / WinRT](../cpp-and-winrt-apis/author-events.md).</span><span class="sxs-lookup"><span data-stu-id="38077-108">To learn how to create a Windows Runtime Component using C++/WinRT, see [Author events in C++/WinRT](../cpp-and-winrt-apis/author-events.md).</span></span>

<span data-ttu-id="38077-109">In dieser exemplarischen Vorgehensweise wird veranschaulicht, wie Sie eine einfache DLL-Datei für eine Komponente für Windows-Runtime erstellen, die von JavaScript, C# oder Visual Basic aufgerufen werden kann.</span><span class="sxs-lookup"><span data-stu-id="38077-109">This walkthrough shows how to create a basic Windows Runtime Component DLL that's callable from JavaScript, C#, or Visual Basic.</span></span> <span data-ttu-id="38077-110">Bevor Sie mit dieser exemplarischen Vorgehensweise beginnen, stellen Sie sicher, dass Sie Konzepte wie die abstrakte binäre Schnittstelle (ABI), Verweisklassen und Visual C++-Komponentenerweiterungen kennen, die das Verwenden von Verweisklassen vereinfachen.</span><span class="sxs-lookup"><span data-stu-id="38077-110">Before you begin this walkthrough, make sure that you understand concepts such as the Abstract Binary Interface (ABI), ref classes, and the Visual C++ Component Extensions that make working with ref classes easier.</span></span> <span data-ttu-id="38077-111">Weitere Informationen finden Sie unter [Erstellen von Komponenten für Windows-Runtime in C++](creating-windows-runtime-components-in-cpp.md) und [Visual C++-Programmiersprachenreferenz (C++/CX)](https://msdn.microsoft.com/library/windows/apps/xaml/hh699871.aspx).</span><span class="sxs-lookup"><span data-stu-id="38077-111">For more information, see [Creating Windows Runtime Components in C++](creating-windows-runtime-components-in-cpp.md) and [Visual C++ Language Reference (C++/CX)](https://msdn.microsoft.com/library/windows/apps/xaml/hh699871.aspx).</span></span>

## <a name="creating-the-c-component-dll"></a><span data-ttu-id="38077-112">Erstellen der C++-Komponenten-DLL-Datei</span><span class="sxs-lookup"><span data-stu-id="38077-112">Creating the C++ component DLL</span></span>
<span data-ttu-id="38077-113">In diesem Beispiel erstellen wir zunächst das Komponentenprojekt. Sie können aber auch das JavaScript-Projekt zuerst erstellen.</span><span class="sxs-lookup"><span data-stu-id="38077-113">In this example, we create the component project first, but you could create the JavaScript project first.</span></span> <span data-ttu-id="38077-114">Die Reihenfolge spielt keine Rolle.</span><span class="sxs-lookup"><span data-stu-id="38077-114">The order doesn’t matter.</span></span>
...
<span data-ttu-id="38077-130">Fügen Sie diese \#include-Direktiven zu „Class1.h“ hinzu:</span><span class="sxs-lookup"><span data-stu-id="38077-130">Add these \#include directives to Class1.h:</span></span>
...
<span data-ttu-id="38077-131">„collection.h“ ist die Headerdatei für konkrete C++-Klassen wie die Platform::Collections::Vector-Klasse und die Platform::Collections::Map-Klasse, die sprachunabhängige, durch die Windows-Runtime definierte Schnittstellen implementieren.</span><span class="sxs-lookup"><span data-stu-id="38077-131">collection.h is the header file for C++ concrete classes such as the Platform::Collections::Vector class and the Platform::Collections::Map class, which implement language-neutral interfaces that are defined by the Windows Runtime.</span></span> <span data-ttu-id="38077-132">Die AMP-Header werden verwendet, um Berechnungen auf der GPU auszuführen.</span><span class="sxs-lookup"><span data-stu-id="38077-132">The amp headers are used to run computations on the GPU.</span></span> <span data-ttu-id="38077-133">Sie haben keine Windows-Runtime-Entsprechungen. Das ist in Ordnung, da sie privat sind.</span><span class="sxs-lookup"><span data-stu-id="38077-133">They have no Windows Runtime equivalents, and that’s fine because they are private.</span></span> <span data-ttu-id="38077-134">Im Allgemeinen sollten Sie aus Leistungsgründen ISO-C++-Code und Standardbibliotheken intern innerhalb der Komponente verwenden. Nur die Windows-Runtime-Schnittstelle muss in Windows-Runtime-Typen ausgedrückt werden.</span><span class="sxs-lookup"><span data-stu-id="38077-134">In general, for performance reasons you should use ISO C++ code and standard libraries internally within the component; it’s just the Windows Runtime interface that must be expressed in Windows Runtime types.</span></span>

## <a name="to-add-a-delegate-at-namespace-scope"></a><span data-ttu-id="38077-135">So fügen Sie einen Delegaten im Gültigkeitsbereich des Namespaces hinzu</span><span class="sxs-lookup"><span data-stu-id="38077-135">To add a delegate at namespace scope</span></span>
<span data-ttu-id="38077-136">Ein Delegat ist ein Konstrukt, das die Parameter und den Rückgabetyp für Methoden definiert.</span><span class="sxs-lookup"><span data-stu-id="38077-136">A delegate is a construct that defines the parameters and return type for methods.</span></span> <span data-ttu-id="38077-137">Ein Ereignis ist eine Instanz eines bestimmten Delegattyps, und eine Ereignishandlermethode, die das Ereignis abonniert, muss über die Signatur verfügen, die im Delegat angegeben wird.</span><span class="sxs-lookup"><span data-stu-id="38077-137">An event is an instance of a particular delegate type, and any event handler method that subscribes to the event must have the signature that's specified in the delegate.</span></span> <span data-ttu-id="38077-138">Der folgende Code definiert einen Delegattyp, der „int“ erhält und „void“ zurückgibt.</span><span class="sxs-lookup"><span data-stu-id="38077-138">The following code defines a delegate type that takes an int and returns void.</span></span> <span data-ttu-id="38077-139">Der Code deklariert danach ein öffentliches Ereignis dieses Typs. Dadurch kann der Clientcode Methoden bereitstellen, die beim Auslösen des Ereignisses aufgerufen werden.</span><span class="sxs-lookup"><span data-stu-id="38077-139">Next the code declares a public event of this type; this enables client code to provide methods that are invoked when the event is fired.</span></span>

<span data-ttu-id="38077-140">Fügen Sie die folgende Delegatdeklaration im Gültigkeitsbereich des Namespaces in „Class1.h“ unmittelbar vor der Class1-Deklaration hinzu.</span><span class="sxs-lookup"><span data-stu-id="38077-140">Add the following delegate declaration at namespace scope in Class1.h, just before the Class1 declaration.</span></span>

```cpp
public delegate void PrimeFoundHandler(int result);
```

<span data-ttu-id="38077-141">Wenn der Code beim Einfügen in Visual Studio nicht korrekt ausgerichtet wird, drücken Sie STRG+K+D, um den Einzug für die gesamte Datei zu korrigieren.</span><span class="sxs-lookup"><span data-stu-id="38077-141">If the code isn’t lining up correctly when you paste it into Visual Studio, just press Ctrl+K+D to fix the indentation for the entire file.</span></span>

## <a name="to-add-the-public-members"></a><span data-ttu-id="38077-142">So fügen Sie öffentliche Member hinzu.</span><span class="sxs-lookup"><span data-stu-id="38077-142">To add the public members</span></span>
<span data-ttu-id="38077-143">Die Klasse stellt drei öffentliche Methoden und ein öffentliches Ereignis bereit.</span><span class="sxs-lookup"><span data-stu-id="38077-143">The class exposes three public methods and one public event.</span></span> <span data-ttu-id="38077-144">Die erste Methode ist synchron, da sie immer sehr schnell ausgeführt wird.</span><span class="sxs-lookup"><span data-stu-id="38077-144">The first method is synchronous because it always executes very fast.</span></span> <span data-ttu-id="38077-145">Da die anderen beiden Methoden einige Zeit in Anspruch nehmen können, sind sie asynchron, damit sie den UI-Thread nicht blockieren.</span><span class="sxs-lookup"><span data-stu-id="38077-145">Because the other two methods might take some time, they are asynchronous so that they don’t block the UI thread.</span></span> <span data-ttu-id="38077-146">Diese Methoden geben IAsyncOperationWithProgress und IAsyncActionWithProgress zurück.</span><span class="sxs-lookup"><span data-stu-id="38077-146">These methods return IAsyncOperationWithProgress and IAsyncActionWithProgress.</span></span> <span data-ttu-id="38077-147">„IAsyncOperationWithProgress“ definiert eine asynchrone Methode, die ein Ergebnis zurückgibt, und „IAsyncActionWithProgress“ definiert eine asynchrone Methode, die „void“ zurückgibt.</span><span class="sxs-lookup"><span data-stu-id="38077-147">The former defines an async method that returns a result, and the latter defines an async method that returns void.</span></span> <span data-ttu-id="38077-148">Diese Schnittstellen ermöglichen auch, dass der Clientcode Updates zum Status des Vorgangs erhält.</span><span class="sxs-lookup"><span data-stu-id="38077-148">These interfaces also enable client code to receive updates on the progress of the operation.</span></span>
...
```
## <a name="to-add-the-private-members"></a><span data-ttu-id="38077-149">So fügen Sie private Member hinzu</span><span class="sxs-lookup"><span data-stu-id="38077-149">To add the private members</span></span>
<span data-ttu-id="38077-150">Die Klasse enthält drei private Member: zwei Hilfsmethoden für die numerischen Berechnungen und ein CoreDispatcher-Objekt, das verwendet wird, um die Ereignisaufrufe von Workerthreads zurück an den UI-Thread zu marshallen.</span><span class="sxs-lookup"><span data-stu-id="38077-150">The class contains three private members: two helper methods for the numeric computations and a CoreDispatcher object that’s used to marshal the event invocations from worker threads back to the UI thread.</span></span>
...
## <a name="to-add-the-implementation-for-computeresult"></a><span data-ttu-id="38077-154">So fügen Sie die Implementierung für ComputeResult hinzu</span><span class="sxs-lookup"><span data-stu-id="38077-154">To add the implementation for ComputeResult</span></span>
<span data-ttu-id="38077-155">Fügen Sie in „Class1.cpp“ die folgende Methodenimplementierung hinzu.</span><span class="sxs-lookup"><span data-stu-id="38077-155">In Class1.cpp, add the following method implementation.</span></span> <span data-ttu-id="38077-156">Diese Methode wird synchron auf dem aufrufenden Thread ausgeführt, ist aber sehr schnell, da sie C++ AMP verwendet, um die Berechnung auf der GPU zu parallelisieren.</span><span class="sxs-lookup"><span data-stu-id="38077-156">This method executes synchronously on the calling thread, but it is very fast because it uses C++ AMP to parallelize the computation on the GPU.</span></span> <span data-ttu-id="38077-157">Weitere Informationen finden Sie in der C++ AMP-Übersicht.</span><span class="sxs-lookup"><span data-stu-id="38077-157">For more information, see C++ AMP Overview.</span></span> <span data-ttu-id="38077-158">Die Ergebnisse werden an einen konkreten Platform::Collections::Vector<T>-Typ angefügt, der bei der Rückgabe implizit in einen Windows::Foundation::Collections::IVector<T>-Typ konvertiert wird.</span><span class="sxs-lookup"><span data-stu-id="38077-158">The results are appended to a Platform::Collections::Vector<T> concrete type, which is implicitly converted to a Windows::Foundation::Collections::IVector<T> when it is returned.</span></span>
...
```
## <a name="to-add-the-implementation-for-getprimesordered-and-its-helper-method"></a><span data-ttu-id="38077-159">So fügen Sie die Implementierung für „GetPrimesOrdered“ und die Hilfsmethode hinzu</span><span class="sxs-lookup"><span data-stu-id="38077-159">To add the implementation for GetPrimesOrdered and its helper method</span></span>
<span data-ttu-id="38077-160">Fügen Sie die Implementierungen für „GetPrimesOrdered“ und die Hilfsmethode „is_prime“ in „Class1.cpp“ hinzu.</span><span class="sxs-lookup"><span data-stu-id="38077-160">In Class1.cpp, add the implementations for GetPrimesOrdered and the is_prime helper method.</span></span> <span data-ttu-id="38077-161">„GetPrimesOrdered“ verwendet eine cConcurrent_vector-Klasse und eine parallel_for-Funktionsschleife zur Aufteilung der Arbeit und um die maximalen Ressourcen des Computers zu verwenden, auf dem das Programm ausgeführt wird, um Ergebnisse zu erzielen.</span><span class="sxs-lookup"><span data-stu-id="38077-161">GetPrimesOrdered uses a concurrent_vector class and a parallel_for function loop to divide up the work and use the maximum resources of the computer on which the program is running to produce results.</span></span> <span data-ttu-id="38077-162">Nachdem die Ergebnisse berechnet, gespeichert und sortiert wurden, werden sie einem Platform::Collections::Vector<T>-Typ hinzugefügt und als „Windows::Foundation::Collections::IVector<T>“ an den Clientcode zurückgegeben.</span><span class="sxs-lookup"><span data-stu-id="38077-162">After the results are computed, stored, and sorted, they are added to a Platform::Collections::Vector<T> and returned as Windows::Foundation::Collections::IVector<T> to client code.</span></span>

<span data-ttu-id="38077-163">Beachten Sie den Code für den Statusbericht, der es dem Client ermöglicht, den Benutzer durch eine Statusanzeige oder ein anderes UI-Element darüber zu informieren, wie lange der Vorgang noch dauern wird.</span><span class="sxs-lookup"><span data-stu-id="38077-163">Notice the code for the progress reporter, which enables the client to hook up a progress bar or other UI to show the user how much longer the operation is going to take.</span></span> <span data-ttu-id="38077-164">Statusberichte sind allerdings nicht kostenlos.</span><span class="sxs-lookup"><span data-stu-id="38077-164">Progress reporting has a cost.</span></span> <span data-ttu-id="38077-165">Ein Ereignis muss auf der Komponentenseite ausgelöst und im UI-Thread behandelt werden, und der Statuswert muss bei jeder Iteration gespeichert werden.</span><span class="sxs-lookup"><span data-stu-id="38077-165">An event must be fired on the component side and handled on the UI thread, and the progress value must be stored on each iteration.</span></span> <span data-ttu-id="38077-166">Eine Möglichkeit, die Kosten zu minimieren, ist das Begrenzen der Häufigkeit, mit der ein Statusereignis ausgelöst wird.</span><span class="sxs-lookup"><span data-stu-id="38077-166">One way to minimize the cost is by limiting the frequency at which a progress event is fired.</span></span> <span data-ttu-id="38077-167">Wenn die Kosten weiterhin unerschwinglich sind oder Sie die Länge des Vorgangs nicht einschätzen können, können Sie einen Statusring verwenden, der anzeigt, dass ein Vorgang ausgeführt wird, der aber nicht die restliche Zeit bis zum Abschluss anzeigt.</span><span class="sxs-lookup"><span data-stu-id="38077-167">If the cost is still prohibitive, or if you can't estimate the length of the operation, then consider using a progress ring, which shows that an operation is in progress but doesn't show time remaining until completion.</span></span>
...
## <a name="to-add-the-implementation-for-getprimesunordered"></a><span data-ttu-id="38077-168">So fügen Sie die Implementierung für „GetPrimesUnordered“ hinzu</span><span class="sxs-lookup"><span data-stu-id="38077-168">To add the implementation for GetPrimesUnordered</span></span>
<span data-ttu-id="38077-169">Der letzte Schritt zum Erstellen der C++-Komponente ist das Hinzufügen der Implementierung für „GetPrimesUnordered“ in „Class1.cpp“.</span><span class="sxs-lookup"><span data-stu-id="38077-169">The last step to create the C++ component is to add the implementation for the GetPrimesUnordered in Class1.cpp.</span></span> <span data-ttu-id="38077-170">Diese Methode gibt jedes einzelne Ergebnis sofort beim Auffinden zurück – ohne zu warten, bis alle Ergebnisse gefunden wurden.</span><span class="sxs-lookup"><span data-stu-id="38077-170">This method returns each result as it is found, without waiting until all results are found.</span></span> <span data-ttu-id="38077-171">Jedes Ergebnis wird im Ereignishandler zurückgegeben und auf der Benutzeroberfläche in Echtzeit angezeigt.</span><span class="sxs-lookup"><span data-stu-id="38077-171">Each result is returned in the event handler and displayed on the UI in real time.</span></span> <span data-ttu-id="38077-172">Beachten Sie auch hier, dass ein Statusbericht verwendet wird.</span><span class="sxs-lookup"><span data-stu-id="38077-172">Again, notice that a progress reporter is used.</span></span> <span data-ttu-id="38077-173">Diese Methode verwendet auch die Hilfsmethode „is_prime“.</span><span class="sxs-lookup"><span data-stu-id="38077-173">This method also uses the is_prime helper method.</span></span>
...
## <a name="to-add-styles"></a><span data-ttu-id="38077-187">So fügen Sie Formate hinzu</span><span class="sxs-lookup"><span data-stu-id="38077-187">To add styles</span></span>
<span data-ttu-id="38077-188">Entfernen Sie in „default.css“ das body-Format, und fügen Sie dann diese Formate hinzu:</span><span class="sxs-lookup"><span data-stu-id="38077-188">In default.css, remove the body style and then add these styles:</span></span>
...
## <a name="to-add-the-javascript-event-handlers-that-call-into-the-component-dll"></a><span data-ttu-id="38077-189">So fügen Sie die JavaScript-Ereignishandler hinzu, die die Komponenten-DLL aufrufen</span><span class="sxs-lookup"><span data-stu-id="38077-189">To add the JavaScript event handlers that call into the component DLL</span></span>
<span data-ttu-id="38077-190">Fügen Sie am Ende der Datei „default.js“ die folgenden Funktionen hinzu.</span><span class="sxs-lookup"><span data-stu-id="38077-190">Add the following functions at the end of the default.js file.</span></span> <span data-ttu-id="38077-191">Diese Funktionen werden aufgerufen, wenn die Schaltflächen auf der Hauptseite ausgewählt werden.</span><span class="sxs-lookup"><span data-stu-id="38077-191">These functions are called when the buttons on the main page are chosen.</span></span> <span data-ttu-id="38077-192">Beachten Sie, wie JavaScript die C++-Klasse aktiviert und dann ihre Methoden aufruft und die Rückgabewerte zum Auffüllen der HTML-Beschriftungen verwendet.</span><span class="sxs-lookup"><span data-stu-id="38077-192">Notice how JavaScript activates the C++ class, and then calls its methods and uses the return values to populate the HTML labels.</span></span>
...
<span data-ttu-id="38077-193">Fügen Sie Code zum Hinzufügen von Ereignis-Listenern durch Ersetzen des vorhandenen Aufrufs von „WinJS.UI.processAll“ in „app.onactivated“ in „default.js“ durch den folgenden Code hinzu, der die Ereignisregistrierung in einem „then“-Block implementiert.</span><span class="sxs-lookup"><span data-stu-id="38077-193">Add code to add the event listeners by replacing the existing call to WinJS.UI.processAll in app.onactivated in default.js with the following code that implements event registration in a then block.</span></span> <span data-ttu-id="38077-194">Eine ausführliche Erläuterung dazu finden Sie unter Erstellen der App „Hello World“ (JS).</span><span class="sxs-lookup"><span data-stu-id="38077-194">For a detailed explanation of this, see Create a "Hello World" app (JS).</span></span>
...
<span data-ttu-id="38077-195">Drücken Sie F5, um die App auszuführen.</span><span class="sxs-lookup"><span data-stu-id="38077-195">Press F5 to run the app.</span></span>
...
## <a name="to-add-the-event-handlers-for-the-buttons"></a><span data-ttu-id="38077-208">So fügen Sie die Ereignishandler für die Schaltflächen hinzu</span><span class="sxs-lookup"><span data-stu-id="38077-208">To add the event handlers for the buttons</span></span>
<span data-ttu-id="38077-209">Öffnen Sie die Datei „MainPage.xaml.cs“ im Projektmappen-Explorer.</span><span class="sxs-lookup"><span data-stu-id="38077-209">In Solution Explorer, open MainPage.xaml.cs.</span></span> <span data-ttu-id="38077-210">(Die Datei befindet sich möglicherweise unter „MainPage.xaml“.) Fügen Sie eine using-Direktive für „System.Text“ und dann den Ereignishandler für die Logarithmusberechnung in der „MainPage“-Klasse hinzu.</span><span class="sxs-lookup"><span data-stu-id="38077-210">(The file might be nested under MainPage.xaml.) Add a using directive for System.Text, and then add the event handler for the Logarithm calculation in the MainPage class.</span></span>
...
<span data-ttu-id="38077-211">Fügen Sie den Ereignishandler für das sortierte Ergebnis hinzu:</span><span class="sxs-lookup"><span data-stu-id="38077-211">Add the event handler for the ordered result:</span></span>
...
<span data-ttu-id="38077-212">Fügen Sie den Ereignishandler für das unsortierte Ergebnis und für die Schaltfläche hinzu, die die Ergebnisse löscht, sodass Sie den Code erneut ausführen können.</span><span class="sxs-lookup"><span data-stu-id="38077-212">Add the event handler for the unordered result, and for the button that clears the results so that you can run the code again.</span></span>
...
## <a name="running-the-app"></a><span data-ttu-id="38077-213">Ausführen der App</span><span class="sxs-lookup"><span data-stu-id="38077-213">Running the app</span></span>
<span data-ttu-id="38077-214">Wählen Sie das C#-Projekt oder das JavaScript-Projekt als Startprojekt aus, indem Sie das Kontextmenü für den Projektknoten im Projektmappen-Explorer öffnen und **Als Startprojekt festlegen** auswählen.</span><span class="sxs-lookup"><span data-stu-id="38077-214">Select either the C# project or JavaScript project as the startup project by opening the shortcut menu for the project node in Solution Explorer and choosing **Set As Startup Project**.</span></span> <span data-ttu-id="38077-215">Drücken Sie dann F5 zum Ausführen mit Debugging oder STRG+F5 zum Ausführen ohne Debugging.</span><span class="sxs-lookup"><span data-stu-id="38077-215">Then press F5 to run with debugging, or Ctrl+F5 to run without debugging.</span></span>
...
<span data-ttu-id="38077-231">Beim Debuggen einer JavaScript-Lösung mit einer Komponenten-DLL können Sie den Debugger so einstellen, dass er das schrittweise Ausführen von Skripts oder das schrittweise Ausführen von systemeigenem Code in der Komponente, aber nicht beides gleichzeitig ermöglicht.</span><span class="sxs-lookup"><span data-stu-id="38077-231">When you debug a JavaScript solution that has a component DLL, you can set the debugger to enable either stepping through script or stepping through native code in the component, but not both at the same time.</span></span> <span data-ttu-id="38077-232">Um die Einstellung zu ändern, öffnen Sie im Projektmappen-Explorer das Kontextmenü für den JavaScript-Projektknoten, und wählen Sie **Eigenschaften, Debuggen, Debuggertyp** aus.</span><span class="sxs-lookup"><span data-stu-id="38077-232">To change the setting, open the shortcut menu for the JavaScript project node in Solution Explorer and choose **Properties, Debugging, Debugger Type**.</span></span>

<span data-ttu-id="38077-233">Achten Sie darauf, entsprechende Funktionen im Paket-Designer auszuwählen.</span><span class="sxs-lookup"><span data-stu-id="38077-233">Be sure to select appropriate capabilities in the package designer.</span></span> <span data-ttu-id="38077-234">Sie können den Paket-Designer durch Öffnen der Datei „Package.appxmanifest“ öffnen.</span><span class="sxs-lookup"><span data-stu-id="38077-234">You can open the package designer by opening the Package.appxmanifest file.</span></span> <span data-ttu-id="38077-235">Wenn Sie z. B. versuchen, programmgesteuert auf Dateien im Ordner „Bilder“ zuzugreifen, stellen Sie sicher, dass das Kontrollkästchen **Bildbibliothek** im Bereich **Funktionen** des Paket-Designers aktiviert ist.</span><span class="sxs-lookup"><span data-stu-id="38077-235">For example, if you are attempting to programmatically access files in the Pictures folder, be sure to select the **Pictures Library** check box in the **Capabilities** pane of the package designer.</span></span>

<span data-ttu-id="38077-236">Wenn der JavaScript-Code die öffentlichen Eigenschaften oder Methoden in der Komponente nicht erkennt, stellen Sie sicher, dass Sie in JavaScript Camel Casing verwenden.</span><span class="sxs-lookup"><span data-stu-id="38077-236">If your JavaScript code doesn't recognize the public properties or methods in the component, make sure that in JavaScript you are using camel casing.</span></span> <span data-ttu-id="38077-237">Auf die `ComputeResult`-C++-Methode muss beispielsweise in JavaScript als `computeResult` verwiesen werden.</span><span class="sxs-lookup"><span data-stu-id="38077-237">For example, the `ComputeResult` C++ method must be referenced as `computeResult` in JavaScript.</span></span>

<span data-ttu-id="38077-238">Wenn Sie ein C++-Komponentenprojekt für Windows-Runtime aus einer Projektmappe entfernen, müssen Sie den Projektverweis auch aus dem JavaScript-Projekt manuell entfernen.</span><span class="sxs-lookup"><span data-stu-id="38077-238">If you remove a C++ Windows Runtime Component project from a solution, you must also manually remove the project reference from the JavaScript project.</span></span> <span data-ttu-id="38077-239">Andernfalls werden nachfolgende Debug- oder Buildvorgänge verhindert.</span><span class="sxs-lookup"><span data-stu-id="38077-239">Failure to do so prevents subsequent debug or build operations.</span></span> <span data-ttu-id="38077-240">Bei Bedarf können Sie dann einen Assemblyverweis zur DLL-Datei hinzufügen.</span><span class="sxs-lookup"><span data-stu-id="38077-240">If necessary, you can then add an assembly reference to the DLL.</span></span>

## <a name="related-topics"></a><span data-ttu-id="38077-241">Verwandte Themen</span><span class="sxs-lookup"><span data-stu-id="38077-241">Related topics</span></span>
* [<span data-ttu-id="38077-242">Erstellen von Komponenten für Windows-Runtime in C++/CX</span><span class="sxs-lookup"><span data-stu-id="38077-242">Creating Windows Runtime Components in C++/CX</span></span>](creating-windows-runtime-components-in-cpp.md)

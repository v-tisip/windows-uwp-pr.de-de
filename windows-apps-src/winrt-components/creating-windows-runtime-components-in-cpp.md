...
Dieses Thema zeigt, wie Sie C++ / CX nach einer Windows-Runtime-Komponente erstellen, also eine Komponente, die aus einer universellen Windows-app mit c#, Visual Basic, C++ oder Javascript erstellt aufgerufen werden kann.

Es gibt verschiedene Gründe für die Erstellung einer Windows-Runtime-Komponente.
...
Im Allgemeinen, wenn Sie code Ihrer C++ / CX-Komponente, verwenden Sie die reguläre C++-Bibliothek und integrierte Typen außer an der Grenze abstrakte binäre Schnittstelle (ABI), in denen Sie Daten in und aus Code in einem anderen winmd-Paket übergeben. Verwenden Sie Windows-Runtime-Typen und die spezielle Syntax, C++ / CX zum Erstellen und Bearbeiten dieser Typen unterstützt. Darüber hinaus in Ihrer C++ / CX-code, Typen wie Delegaten und Ereignisse, die von der Komponente ausgelöst und verarbeitet werden können in JavaScript, Visual Basic, C++ oder C#-Implementierung verwenden. Weitere Informationen zu C++ / CX-Syntax finden Sie unter [Visual C++-Programmiersprachenreferenz (C++ / CX)](https://msdn.microsoft.com/library/windows/apps/xaml/hh699871.aspx).
...
Wenn Sie einen DateTime-Wert übergeben, von C++ / CX für JavaScript, JavaScript akzeptiert ihn als ein Datum-Objekt und standardmäßig als Datumszeichenfolge Langform angezeigt.
...
Wenn eine .NET-Sprache einen System.DateTime-Wert übergibt, an eine C++ / CX-Komponente, die Methode akzeptiert ihn als eine Windows::Foundation::DateTime. Wenn die Komponente einen Windows::Foundation::DateTime-Wert an eine .NET Framework-Methode übergibt, akzeptiert die Framework-Methode ihn als DateTimeOffset.
...
Die .NET-Sprachen greifen auf Eigenschaften für einen systemeigenen C++ / CX-Objekt genau wie bei einem .NET Framework-Objekt.
...
Eine Windows-Runtime-Enumeration in C++ / CX wird mithilfe der öffentliche Klasse Enum; deklariert Es ähnelt eine Bereichsbezogene Enumeration in Standard-c++.
...
Enumerationswerte werden übergeben zwischen C++ / CX und JavaScript als ganze Zahlen. Sie können optional ein JavaScript-Objekt, das dieselben benannten Werte wie C++ enthält, deklarieren / CX-Enumeration und verwenden es wie folgt.
...
Asynchrone Methoden implementieren von in C++ / CX verwenden Sie die [Create\_async](https://msdn.microsoft.com/library/hh750102.aspx) -Funktion, die in "ppltasks.h" definiert ist. Weitere Informationen finden Sie [Erstellen asynchroner Vorgänge in C++ / CX für UWP-apps](https://msdn.microsoft.com/library/vstudio/hh750082.aspx). Ein Beispiel finden Sie [Exemplarische Vorgehensweise: Erstellen einer einfachen Komponente für Windows-Runtime in C++ / CX und Aufrufen der Komponente über JavaScript oder C#-](walkthrough-creating-a-basic-windows-runtime-component-in-cpp-and-calling-it-from-javascript-or-csharp.md). Die .NET-Sprachen verwenden C++ / CX asynchrone Methoden nur, wie eine asynchrone Methode, die in .NET Framework definiert ist.
...
Wenn der JavaScript-Code die öffentlichen Eigenschaften oder Methoden in der Komponente nicht zu erkennen scheint, stellen Sie sicher, dass Sie in JavaScript die Kamelschreibweise verwenden. Z. B. die LogCalc c++ / CX-Methode muss als LogCalc in JavaScript darauf verwiesen werden.

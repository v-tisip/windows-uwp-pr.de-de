---
title: "Lernprogramm für die ersten Schritte– ein 2D-UWP-Spiel in MonoGame"
description: "Ein einfaches UWP-Spiel für den Windows Store, geschrieben in C# und MonoGame"
author: mukin
ms.author: mukin
ms.date: 03/06/2017
ms.topic: article
...
translationtype: HT
---
# <a name="get-started-tutorial-a-uwp-game-in-monogame-2d"></a>Lernprogramm für die ersten Schritte: ein 2D-UWP-Spiel in MonoGame

## <a name="a-simple-2d-uwp-game-for-the-windows-store-written-in-c-and-monogame"></a>Ein einfaches 2D-UWP-Spiel für den Windows Store, geschrieben in C# und MonoGame
...
+    Windows10 und Microsoft Visual Studio2015 [Klicken Sie hier, um zu erfahren, wie Sie Visual Studio einrichten](https://docs.microsoft.com/en-us/windows/uwp/get-started/get-set-up).
...
Öffnen Sie das Projekt in Visual Studio2015, und drücken Sie **F5**, um das Beispiel auszuführen. Wenn Sie das Projekt zum ersten Mal laden, kann dies möglicherweise etwas länger dauern, da Visual Studio alle NuGet-Pakete abrufen muss, die nicht mit installiert wurden.
...
**Hinweis:** Das in diesem Beispiel erstellte Spiel soll nicht abgeschlossen sein (oder überhaupt Spaß machen). Sein einziger Zweck besteht darin, die grundlegenden Konzepte der 2D-Entwicklung in MonoGame zu veranschaulichen. Verwenden Sie diesen Code und verbessern Sie ihn – oder beginnen Sie einfach ganz von vorn, nachdem Sie die Grundlagen kennengelernt haben!
...
2. Starten Sie Visual Studio2015.
...
Für den Avatar des Spielers verwenden wir die Ninja-Katze von Microsoft, die auf einem riesigen Tyrannosaurus Rex reitet. [Klicken Sie hier, um das Bild herunterzuladen](https://github.com/muhsinking/VeggieJump/blob/master/MonoGame2D/Content/ninja-cat-dino.png).
...
7. Tastatureingaben – Als Nächstes benötigen wir eine neue Methode für den Umgang mit Benutzereingaben über die Tastatur. Fügen Sie diese Methode zu **Game1.cs** hinzu:
...
## <a name="publish-to-the-windows-store"></a>Veröffentlichen im Windows Store
Da wir dieses Spiel als UWP-App erstellt haben, können Sie dieses Projekt im Windows Store veröffentlichen. Dazu müssen Sie einige Schritte durchlaufen.
...
Weitere Informationen finden Sie unter [Veröffentlichen Ihrer Windows Store-App](https://developer.microsoft.com/en-us/store/publish-apps).

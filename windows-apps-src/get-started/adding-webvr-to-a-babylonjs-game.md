...
Die einfachste Möglichkeit für den Einstieg ist, das [Windows-Lernprogramme-Web-GitHub-Repository](https://github.com/Microsoft/Windows-tutorials-web), besuchen, drücken Sie die grüne **Clone or Download** Schaltfläche ", und wählen Sie **in Visual Studio geöffnet**.
...
Die vorher und Nachher Ordner diese Dateien enthalten:
-   **Texturen /** – ein Ordner mit den im Spiel verwendeten Bilder.
-   **Css /** – ein Ordner mit den CSS-Code für das Spiel.
-   **Js /** – ein Ordner mit den JavaScript-Dateien. Die Datei "Main.js" ist unser Spiel, und die anderen Dateien sind die Bibliotheken verwendet.
...
Wenn Sie noch keine Erfahrung mit Windows Mixed Reality und Windows 10 Creators Update auf einem Computer mit einem kompatiblen Grafikkarte installiert haben, versuchen Sie die **Mixed Reality-Portal** -app aus dem Menü "Start" in Windows 10 zu öffnen.

![Mixed Reality-Portal-Suche](images/mixed-reality-portal.png)

Wenn Sie alle Anforderungen erfüllt, können Sie dann Entwicklerfeatures aktivieren und simulieren einer Windows Mixed Reality-Kopfhörer an den Computer angeschlossen. Wenn Sie Glück, einen tatsächlichen Kopfhörer in der Nähe sind, schließen Sie es, und führen Sie das Setup.
...
>[!NOTE]
> Nehmen Sie den Ordner [**vor**](https://github.com/Microsoft/Windows-tutorials-web/tree/master/BabylonJS-game-with-WebVR/before) , um das Beispiel Starter abrufen.
...
Wir müssen Sie zuerst eine [`AdvancedDynamicTexture`](https://doc.babylonjs.com/how_to/gui#advanceddynamictexture) Objekt herstellen, was die GUI behandelt wird. Das Beispiel legt diese auf `CreateFullScreenUI()`, d. h. unsere UI deckt den gesamten Bildschirm. Mit `AdvancedDynamicTexture` erstellt haben, wir stellen ein 2D Text-Feld, das angezeigt beim Starten wird, das Spiel mit `GUI.Rectanlge()` und `GUI.TextBlock()`.
...
Es ist üblich, VR-Anwendungen auf zwei Arten von Kameras verfügen, sodass mehrere Szenarien unterstützt werden können. Für dieses Spiel müssen wir eine Kamera, die erfordert eine funktionierende Kopfhörer zu angeschlossen sein und einen anderen unterstützen, die keine Kopfhörer verwendet. Um zu ermitteln, welche wird das Spiel verwenden müssen wir überprüfen Sie zunächst, ob ein Kopfhörer erkannt wurde. Dazu verwenden wir [`navigator.getVRDisplays()`](https://developer.mozilla.org/en-US/docs/Web/API/Navigator/getVRDisplays).


Fügen Sie diesen Code oben genannten `window.addEventListener('DOMContentLoaded')` in **"Main.js"**.
```javascript
var headset;
...
Mit den Informationen in gespeicherten der `headset` Variable, wir werden jetzt in der Lage, die Kamera auszuwählen, die für den Benutzer geeignet ist.
...
Für unsere fallback Kamera verwenden wir die [`UniversalCamera`](https://doc.babylonjs.com/classes/3.1/universalcamera) , die derzeit in der ursprünglichen Spiel verwendet wird.
...
Um diese Kamera in den meisten Browsern zu aktivieren, muss der Benutzer einige Interaktion mit automatischem, die die virtuelle Umgebung anfordert.
...
Ein Mausklick in das Spiel jetzt erstellt eine Aufforderung wie folgt oder zeigt das Spiel in die Kopfhörer sofort, wenn der Benutzer die Aufforderung vor dem akzeptiert hat.
...
Da die `WebVRFreeCamera` unterstützt keine anfänglich Gamepads, müssen wir die Pfeiltasten der Tastatur unsere Gamepad-Tasten zuordnen. Wir werden dies tun, indem eine Suche in der `inputs` Eigenschaft der Kamera. Durch das Hinzufügen der Codes für die linken analogstick nach oben, unten, links und rechts, um mit den Pfeiltasten übereinstimmen, sind unsere Gamepads zurück in Aktion zu sehen.
...
Wenn wir **"Index.HTML"** mit unseren Kopfhörer öffnen und Gamecontroller angeschlossen, wird ein Klicken Sie auf das blaue Spielfenster unser Spiel VR-Modus wechseln! Fahren Sie fort, und platzieren Sie auf Ihre Kopfhörer auf die Ergebnisse in Betracht ziehen. 
...
Herzlichen Glückwunsch! Sie haben nun ein vollständiges Babylon.js-Spiel mit WebVR Unterstützung. Hier können Sie nutzen, was Sie gelernt haben, erstellen Sie ein noch besseres Spiel oder Deaktivieren dieses erstellt.

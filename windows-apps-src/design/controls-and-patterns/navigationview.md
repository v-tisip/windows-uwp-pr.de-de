...
> **Dies ist eine Vorschauversion**: in diesem Artikel wird beschrieben, eine neue Version des das NavigationView-Steuerelement, das noch in Entwicklung befindet. Um sie zu verwenden benötigen nun die [neuesten Windows-Insider-Build und SDK](https://insider.windows.com/for-developers/) oder der [Windows-UI-Bibliothek](https://docs.microsoft.com/uwp/toolkits/winui/).
...
Andere Navigationssteuerelementen finden Sie unter [navigationsdesigngrundlagen](../basics/navigation-basics.md).
...
Wir empfehlen die linke Navigation beim:

- Sie haben eine mittlere bis hohe Anzahl (5-10) wichtiger Navigation auf oberster Ebene Kategorien.
- Gewünschte sehr wichtigen Navigationskategorien mit weniger Speicherplatz für die anderen Inhalten der app.
...
Wir empfehlen oberen Navigationsleiste beim:

- Sie haben 5 oder weniger wichtiger Navigation auf oberster Ebene Kategorien, sodass Kategorien zusätzliche Navigation auf oberster Ebene, die in der Dropdownliste landen overflow Menü gelten als weniger wichtig.
...
Passt zwischen LeftMinimal auf kleinen Bildschirmen, LeftCompact für mittelgroße Bildschirme und Links auf großen Bildschirmen an. Finden Sie im Abschnitt [adaptives Verhalten](#adaptive-behavior) für Weitere Informationen.
...
Der Bereich kann positioniert werden im Vordergrund oder links, über die `PanePosition` Eigenschaft.
...
Die zurück-Schaltfläche in der oberen linken Ecke des Bereichs, jedoch NavigationView nicht automatisch Inhalte auf die zurück-Stapel hinzu. Zum Aktivieren der Rückwärtsnavigation, finden Sie unter der [Rückwärts Navigation](#backwards-navigation) Abschnitt.
...
### <a name="pane-header"></a>Kopf

Freier Inhalt in den Bereich Header, wenn die Eigenschaft [PaneHeader](https://docs.microsoft.com/uwp/api/windows.ui.xaml.controls.navigationview.PaneHeader) hinzugefügt
...
Freier Inhalt im Bereich, wenn die Eigenschaft [PaneCustomContent](https://docs.microsoft.com/uwp/api/windows.ui.xaml.controls.navigationview.PaneCustomContent) hinzugefügt
...
Ein zweites Allgemeines adaptives Muster ist einen erweiterten linken Bereich auf großen fensterbreiten und ein Hamburger-Menü auf beide kleinen und mittelgroßen fensterbreiten verwenden.
...
In einigen Fällen kann ein adaptives Verhalten nicht auf allen gewünschten. Sie können den Bereich erweitert, immer compact oder immer minimale immer ist festlegen.
...
### <a name="top-to-left-navigation"></a>Von oben links
...
In einigen Fällen müssen apps unterschiedliche Daten an den oberen Bereich und Links zu binden. Im linke Bereich umfasst häufig weitere Elemente für die Navigation.
...
Im Modell Registerkarten sind Auswahl und Fokus gebunden. Eine Aktion, dass in der Regel Schichten auch Auswahl Fokus würde. In dem Beispiel unten haben rechten Arrowing verschoben Marke für die Auswahl aus der Anzeige Bildschirmlupe. Sie können dies erreichen, indem Sie die [SelectionFollowsFocus](https://docs.microsoft.com/uwp/api/windows.ui.xaml.controls.navigationview.selectionfollowsfocus) -Eigenschaft aktiviert.

![Screenshot von nur-Text-oberen navview](images/nav-tabs.png)

Nachfolgend finden Sie im XAML-Beispiel für diese:
...
Um austauschen, um den Inhalt beim Registerkarte Auswahl zu ändern, verwenden Sie Frames [NavigateWithOptions](https://docs.microsoft.com/uwp/api/windows.ui.xaml.controls.frame.NavigateToType) Methode FrameNavigationOptions.IsNavigationStackEnabled auf "false" festgelegt und NavigateOptions.TransitionInfoOverride festlegen, an der entsprechenden Seite-zu-Seite Folienanimation. Ein Beispiel finden Sie im folgenden [Codebeispiel wird](#code-example) .
...
Im folgenden ist ein End-to-End-Beispiel dafür, wie Sie NavigationView mit ein oberer Navigationsbereich auf großen Fenstergrößen und einem linken Navigationsbereich auf kleinen Fenstergrößen integrieren können.

In diesem Beispiel werden wir davon ausgehen, dass Endbenutzer häufig neue Navigationskategorien, auswählen, und wir:
...
- Halten Sie den Standardwert für die [ShoulderNavigationEnabled](https://docs.microsoft.com/uwp/api/windows.ui.xaml.controls.navigationview.PanePostion) -Eigenschaft, die verwendet wird, um anzugeben, ob linker/rechter Bumper auf einem Gamepad die Navigation auf oberster Ebene Kategorien Ihrer App navigieren. Der Standardwert ist "WhenSelectionFollowsFocus". Andere mögliche Werte sind "Immer" und "Nie".

Es wird veranschaulicht, wie Sie die Rückwärtsnavigation mit der zurück-Schaltfläche implementiert.

Hier sehen Sie eine Aufzeichnung der Zweck des Beispiels:
...
Der Hintergrund des Bereichs zeigt in-app-Acryl, wenn sich NavigationView im oberen, minimale oder kompakten Modus befindet. Um dieses Verhalten zu aktualisieren oder die Darstellung des Acrylbereichs anzupassen, ändern Sie die zwei Designressourcen durch Überschreiben Ihrer App.xaml.
...
Für eine nahtlose aussehen + Verhalten wird empfohlen, wenn Ihre app-Seiten, die einem ScrollViewer und Navigationsbereich Anfang positioniert wird, müssen den Bildlauf unterhalb der oberen Navigationsbereich.

Dies kann erreicht werden, indem Sie die [CanContentRenderOutsideBounds](https://docs.microsoft.com/uwp/api/windows.ui.xaml.controls.scrollviewer.cancontentrenderoutsidebounds) -Eigenschaft im entsprechenden ScrollViewer auf "true".
...
Sie können dies erreichen, indem Sie die [ContentOverlay](https://docs.microsoft.com/uwp/api/windows.ui.xaml.controls.navigationview.ContentOverlay) -Eigenschaft auf NavigationView. 

In einigen Fällen, wenn der Benutzer nach unten Bildläufe ist, können Sie den Navigationsbereich erreicht, indem Sie die [IsPaneVisible](https://docs.microsoft.com/uwp/api/windows.ui.xaml.controls.navigationview.ContentOverlay) -Eigenschaft auf NavigationView auf "false" ausblenden möchten.

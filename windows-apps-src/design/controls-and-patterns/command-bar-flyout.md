...
ms.locfileid: "4318943"
---
# <a name="command-bar-flyout"></a>Befehlsleiste flyout

Das Befehlsleiste Flyout können Sie Benutzer einfachen Zugriff auf allgemeine Aufgaben bereitstellen, indem Befehle in einem schwebenden Symbolleisten im Zusammenhang mit einem Element auf die UI-Canvas angezeigt.
...
Proaktive Aufruf werden Befehle automatisch angezeigt, wenn der Benutzer mit dem Element interagiert, die die Befehle zugeordnet sind. Z. B. möglicherweise Formatierungsbefehle Text eingeblendet, wenn der Benutzer Text in einem Textfeld auswählt. In diesem Fall wird das Befehlsleiste Flyout Fokus nicht verwendet. Stattdessen zeigt sie geeignete Befehle nahe dem Element, dem der Benutzer mit interagiert. Wenn der Benutzer mit den Befehlen interagieren nicht, werden sie geschlossen.

Reaktive Aufruf werden Befehle in Reaktion auf eine explizite Benutzeraktion angezeigt, die Befehle anfordern. Beispiel: eine mit der rechten Maustaste. Dies entspricht dem herkömmliche Konzept eines [Kontextmenüs](menus.md).
...
> [!NOTE]
> Um die primären oder sekundären Befehl Sammlungen Befehlsleiste Flyout hinzugefügt werden, muss ein Element die [ICommandBarElement](/uwp/api/windows.ui.xaml.controls.icommandbarelement) -Schnittstelle implementieren. AppBarElementContainer ist ein Wrapper, der diese Schnittstelle implementiert, sodass Sie ein Element zu einer Befehlsleiste hinzufügen können, selbst wenn sie nicht die Schnittstelle selbst implementiert.
...
> [!NOTE]
> Dieses Beispiel zeigt nur dem Befehlsleiste Flyout UI, sie keine Befehle implementiert, die angezeigt werden. Weitere Informationen zur Implementierung der Befehle finden Sie unter [Schaltflächen](buttons.md) und [befehlsdesigngrundlagen](../basics/commanding-basics.md).
...
| Einfügen | Wenn das Text-Steuerelement ist nicht schreibgeschützt, und die Zwischenablage Inhalt ist. |

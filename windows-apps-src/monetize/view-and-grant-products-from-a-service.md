...
2.  [Zuordnen Ihrer Azure AD-Anwendung-ID mit Ihrer app im Windows Dev Center-Dashboard](#step-2).
...
* **Ihr Dienst**. Dies ist eine Anwendung, die sicher im Kontext Ihrer geschäftlichen Umgebung ausgeführt, und alle Entwicklungsplattform gewählte mit implementiert werden kann. Der Dienst ist verantwortlich für die Azure AD-Zugriffstoken erstellen für das Szenario und für den Aufruf der REST-URIs für die Microsoft Store-Sammlungs-API und -Einkaufs-API benötigt.
...
4.  Erstellen Sie einen Schlüssel, die Sie später müssen (Dies wird alle bezeichnet einen *geheimen Clientschlüssel*). Klicken Sie im linken Bereich auf **Einstellungen** , und klicken Sie dann **Schlüssel**. Auf dieser Seite die Schritte zum [Erstellen eines Schlüssels](https://docs.microsoft.com/azure/active-directory/develop/active-directory-integrating-applications#to-add-application-credentials-or-permissions-to-access-web-apis). Kopieren Sie diesen Schlüssel zur späteren Verwendung.
...
2.  Wechseln Sie zu der **Dienste** &gt; **produktsammlungen und Einkäufe** Seite, und geben Sie Ihre Azure AD-Anwendungs-ID in eines der verfügbaren **Client-ID** -Felder.

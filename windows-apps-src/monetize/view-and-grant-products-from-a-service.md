...
Bevor Sie mit der Microsoft Store-Sammlungs-API oder --API Einkaufs, müssen Sie eine Azure AD-Webanwendung erstellen, die Mandanten-ID und Anwendungs-ID für die Anwendung abrufen und einen Schlüssel generieren. Die Azure AD-Webanwendung steht für den Dienst aus dem Aufrufen der Microsoft Store-Sammlungs-API oder -Einkaufs-API werden soll. Sie benötigen die Mandanten-ID, Anwendungs-ID und Schlüssel Azure AD-Zugriffstoken generieren, die Sie benötigen, um die API aufrufen.
...
3.  Die Anwendung wichtigsten Registrierungsseite wird aufgerufen. Kopieren Sie auf dieser Seite die **Anwendungs-ID** -Wert zur späteren Verwendung.
...
Bevor Sie die Microsoft Store-Sammlungs-API verwenden oder --API Einkaufs, um die Gesamtbetriebskosten und für Ihre app oder Add-on-Käufe konfigurieren, müssen Sie zuordnen Ihrer Azure AD-Anwendung-ID der app (oder die app, die das Add-on enthält) im Partner Center.

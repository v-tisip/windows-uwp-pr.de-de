...
## <a name="display-the-graphics-by-rendering-the-frame"></a><span data-ttu-id="60d26-136">Anzeigen die Grafiken: Rendern des Frames</span><span class="sxs-lookup"><span data-stu-id="60d26-136">Display the graphics by rendering the frame</span></span>
...
### <a name="primitive-rendering"></a><span data-ttu-id="60d26-165">Rendern der Grundtypen</span><span class="sxs-lookup"><span data-stu-id="60d26-165">Primitive rendering</span></span>
...
<span data-ttu-id="60d26-180">Gründe für die Verwendung mehrerer Konstantenpuffer:</span><span class="sxs-lookup"><span data-stu-id="60d26-180">Reasons for multiple constant buffers:</span></span>
...
### <a name="present"></a><span data-ttu-id="60d26-191">Darstellen</span><span class="sxs-lookup"><span data-stu-id="60d26-191">Present</span></span>
...
## <a name="terms-and-concepts"></a><span data-ttu-id="60d26-210">Begriffe und Konzepte</span><span class="sxs-lookup"><span data-stu-id="60d26-210">Terms and concepts</span></span>
...
<span data-ttu-id="60d26-224">Um diese Pipeline erstellen zu können, müssen Sie damit vertraut sein:</span><span class="sxs-lookup"><span data-stu-id="60d26-224">To help you create this pipeline, you need to be familiar with:</span></span>
...
#### <a name="various-shader-file-formats"></a><span data-ttu-id="60d26-255">Verschiedene Shader-Dateiformate</span><span class="sxs-lookup"><span data-stu-id="60d26-255">Various shader file formats</span></span>
...
<span data-ttu-id="60d26-285">Tiefeninformationen steuern, welche Bereiche von Polygonen dargestellt werden.</span><span class="sxs-lookup"><span data-stu-id="60d26-285">Depth info tells us which areas of polygons are rendered rather than hidden from view.</span></span> <span data-ttu-id="60d26-286">Schabloneninformationen steuern, welche Pixel maskiert werden.</span><span class="sxs-lookup"><span data-stu-id="60d26-286">Stencil info tells us which pixels are masked.</span></span> <span data-ttu-id="60d26-287">Es kann verwendet werden, um Spezialeffekte zu erzeugen, da es bestimmt, ob ein Pixel gezeichnet oder nicht gezeichnet wird. Es setzt das Bit auf 1 oder 0.</span><span class="sxs-lookup"><span data-stu-id="60d26-287">It can be used to produce special effects since it determines whether a pixel is drawn or not; sets the bit to a 1 or 0.</span></span> 
...
#### <a name="design-and-implementation-of-buffers"></a><span data-ttu-id="60d26-333">Entwurf und -Implementierung der Puffer</span><span class="sxs-lookup"><span data-stu-id="60d26-333">Design and implementation of buffers</span></span>
...
<span data-ttu-id="60d26-368">Das Spiel enthält den erforderlichen Code, um die Spielwelt in seinem eigenen Koordinatensystem (auch als Spielweltbereich oder Szenenbereich bezeichnet) zu aktualisieren.</span><span class="sxs-lookup"><span data-stu-id="60d26-368">The game has the code in place to update the world in its own coordinate system (sometimes called the world space or scene space).</span></span> <span data-ttu-id="60d26-369">Alle Objekte, einschließlich der Kamera, werden in diesem Bereich positioniert und ausgerichtet.</span><span class="sxs-lookup"><span data-stu-id="60d26-369">All objects, including the camera, are positioned and oriented in this space.</span></span> <span data-ttu-id="60d26-370">Weitere Informationen zu Standortsystemen finden Sie unter [Koordinatensysteme](../graphics-concepts/coordinate-systems.md)..</span><span class="sxs-lookup"><span data-stu-id="60d26-370">For more information, see [Coordinate systems](../graphics-concepts/coordinate-systems.md).</span></span>

<span data-ttu-id="60d26-371">Ein Vertexshader übernimmt die schwere Aufgabe, die Modellkoordinaten mit dem folgenden Algorithmus in die Gerätekoordinaten zu konvertieren (wobei V ein Vektor und M eine Matrix ist).</span><span class="sxs-lookup"><span data-stu-id="60d26-371">A vertex shader does the heavy lifting of converting from the model coordinates to device coordinates with the following algorithm (where V is a vector and M is a matrix).</span></span>

<span data-ttu-id="60d26-372">V(Gerät) = V(Modell) x M(model-to-world) x M(world-to-view) x M(view-to-device).</span><span class="sxs-lookup"><span data-stu-id="60d26-372">V(device) = V(model) x M(model-to-world) x M(world-to-view) x M(view-to-device).</span></span>

<span data-ttu-id="60d26-373">Dabei gilt Folgendes:</span><span class="sxs-lookup"><span data-stu-id="60d26-373">where:</span></span> 
* <span data-ttu-id="60d26-374">M(Model-to-World) ist eine Transformationsmatrix für die Modellkoordinaten in Spielweltkoordinaten, auch bekannt als die [World transform matrix](#world-transform-matrix).</span><span class="sxs-lookup"><span data-stu-id="60d26-374">M(model-to-world) is a transformation matrix for model coordinates to world coordinates, also known as the [World transform matrix](#world-transform-matrix).</span></span> <span data-ttu-id="60d26-375">Diese Matrix wird vom Grundtyp bereitgestellt.</span><span class="sxs-lookup"><span data-stu-id="60d26-375">This is provided by the primitive.</span></span>
* <span data-ttu-id="60d26-376">M(world-to-view) ist eine Transformationsmatrix für die Weltkoordinaten in Spielansichtskoordinaten, auch bekannt als die [Ansichtstransformationsmatrix](#view-transform-matrix).</span><span class="sxs-lookup"><span data-stu-id="60d26-376">M(world-to-view) is a transformation matrix for world coordinates to view coordinates, also known as the [View transform matrix](#view-transform-matrix).</span></span>
    * <span data-ttu-id="60d26-377">Diese Matrix wird von der Ansichtsmatrix der Kamera bereitgestellt.</span><span class="sxs-lookup"><span data-stu-id="60d26-377">This is provided by the view matrix of the camera.</span></span> <span data-ttu-id="60d26-378">Es wird durch die Position der Kamera und die Blickvektoren definiert (der Vektor "anschauen", der von der Kamera in die Szene weist, und der Vektor "nach oben schauen" senkrecht zur Szene).</span><span class="sxs-lookup"><span data-stu-id="60d26-378">It's defined by the camera's position along with the look vectors (the "look at" vector that points directly into the scene from the camera, and the "look up" vector that is upwards perpendicular to it)</span></span>
    * <span data-ttu-id="60d26-379">Im Beispielspiel ist __m\_viewMatrix__ die Ansichtstransformationsmatrix und wird mit __Camera::SetViewParams__ berechnet</span><span class="sxs-lookup"><span data-stu-id="60d26-379">In the sample game, __m\_viewMatrix__ is the view transformation matrix and is calculated using __Camera::SetViewParams__</span></span> 
* <span data-ttu-id="60d26-380">M(view-to-device) ist eine Transformationsmatrix für die Ansichtskoordinaten in Gerätekoordinaten, auch bekannt als die [Projektstransformationsmatrix](#projection-transform-matrix).</span><span class="sxs-lookup"><span data-stu-id="60d26-380">M(view-to-device) is a transformation matrix for view coordinates to device coordinates, also known as the [Projection transform matrix](#projection-transform-matrix)</span></span>
    * <span data-ttu-id="60d26-381">Diese Matrix wird von der Projektion der Kamera bereitgestellt.</span><span class="sxs-lookup"><span data-stu-id="60d26-381">This is provided by the projection of the camera.</span></span> <span data-ttu-id="60d26-382">Es enthält Informationen darüber, wie dieser Platz tatsächlich in der endgültigen Szene sichtbar ist.</span><span class="sxs-lookup"><span data-stu-id="60d26-382">It provides info how much of that space is actually visible in the final scene.</span></span> <span data-ttu-id="60d26-383">Das Blickfeld (FoV), das Seitenverhältnis und die Clippingebenen definieren die Projektionstransformationsmatrix.</span><span class="sxs-lookup"><span data-stu-id="60d26-383">The Field of View (FoV), aspect ratio, and clipping planes define the projection transform matrix.</span></span>
    * <span data-ttu-id="60d26-384">Im Beispielspiel definiert __m\_projectionMatrix__ die Transformation der Projektionskoordinaten, berechnet mithilfe von __Camera::SetProjParams__ (für die Stereoprojektion verwenden Sie zwei Projektionsmatrizen: eine für die Ansicht jedes Auges.)</span><span class="sxs-lookup"><span data-stu-id="60d26-384">In the sample game, __m\_projectionMatrix__ defines transformation to the projection coordinates, calculated using __Camera::SetProjParams__ (For stereo projection, you use two projection matrices: one for each eye's view.)</span></span> 

<span data-ttu-id="60d26-385">Der Shadercode in VertexShader.hlsl wird mit diesen Vektoren und Matrizen aus den Konstantenpuffern geladen und führt diese Transformation für jeden Vertex aus.</span><span class="sxs-lookup"><span data-stu-id="60d26-385">The shader code in VertexShader.hlsl is loaded with these vectors and matrices from the constant buffers, and performs this transformation for every vertex.</span></span>

### <a name="coordinate-transformation"></a><span data-ttu-id="60d26-386">Koordinatentransformation</span><span class="sxs-lookup"><span data-stu-id="60d26-386">Coordinate transformation</span></span>

<span data-ttu-id="60d26-387">Direct3D verwendet drei Transformationen, um Ihre 3D-Modell-Koordinaten in Pixelkoordinaten zu verwandeln (Bildschirmbereich).</span><span class="sxs-lookup"><span data-stu-id="60d26-387">Direct3D uses three transformations to change your 3D model coordinates into pixel coordinates (screen space).</span></span> <span data-ttu-id="60d26-388">Diese Transformationen sind die globale Transformation, die Ansichtstransformation und Projektionstransformation.</span><span class="sxs-lookup"><span data-stu-id="60d26-388">These transformations are world transform, view transform, and projection transform.</span></span> <span data-ttu-id="60d26-389">Weitere Informationen finden Sie unter [Übersicht über Transformationen](../graphics-concepts/transform-overview.md).</span><span class="sxs-lookup"><span data-stu-id="60d26-389">For more info, go to [Transform overview](../graphics-concepts/transform-overview.md).</span></span>

#### <a name="world-transform-matrix"></a><span data-ttu-id="60d26-390">Welttransformationsmatrix</span><span class="sxs-lookup"><span data-stu-id="60d26-390">World transform matrix</span></span>

<span data-ttu-id="60d26-391">Eine Welttransformationsmatrix ändert die Koordinaten des Modellbereichs, in dem Scheitelpunkte definiert werden, die relativ zum lokalen Ursprung des Modells im Weltraum sind, wo Scheitelpunkte relativ zum Ursprung für alle Objekte in einer Szene definiert werden.</span><span class="sxs-lookup"><span data-stu-id="60d26-391">A world transform changes coordinates from model space, where vertices are defined relative to a model's local origin, to world space, where vertices are defined relative to an origin common to all the objects in a scene.</span></span> <span data-ttu-id="60d26-392">Die Welttransformation platziert ein Modell in der Welt, daher ihr Name.</span><span class="sxs-lookup"><span data-stu-id="60d26-392">In essence, the world transform places a model into the world; hence its name.</span></span> <span data-ttu-id="60d26-393">Weitere Informationen finden Sie unter [Welttransformationen](../graphics-concepts/world-transform.md).</span><span class="sxs-lookup"><span data-stu-id="60d26-393">For more information, see [World transform](../graphics-concepts/world-transform.md).</span></span>

####  <a name="view-transform-matrix"></a><span data-ttu-id="60d26-394">Ansichtstransformationsmatrix</span><span class="sxs-lookup"><span data-stu-id="60d26-394">View transform matrix</span></span>

<span data-ttu-id="60d26-395">Die Ansichtstransformation lokalisiert den Betrachter im Weltbereich und transformiert dazu Scheitelpunkte in den Kamerabereich.</span><span class="sxs-lookup"><span data-stu-id="60d26-395">The view transform locates the viewer in world space, transforming vertices into camera space.</span></span> <span data-ttu-id="60d26-396">Im Kamerabereich befindet sich die Kamera, bzw. der Betrachter, am Ursprungspunkt und blickt in die positive z-Richtung.</span><span class="sxs-lookup"><span data-stu-id="60d26-396">In camera space, the camera, or viewer, is at the origin, looking in the positive z-direction.</span></span> <span data-ttu-id="60d26-397">Weitere Informationen finden Sie unter [Ansichtstransformation](../graphics-concepts/view-transform.md).</span><span class="sxs-lookup"><span data-stu-id="60d26-397">For more info, go to [View transform](../graphics-concepts/view-transform.md).</span></span>

####  <a name="projection-transform-matrix"></a><span data-ttu-id="60d26-398">Projektionstransformationsmatrix</span><span class="sxs-lookup"><span data-stu-id="60d26-398">Projection transform matrix</span></span>

<span data-ttu-id="60d26-399">Die Projektionstransformation konvertiert das Ansichtsfrustum in eine Quaderform.</span><span class="sxs-lookup"><span data-stu-id="60d26-399">The projection transform converts the viewing frustum to a cuboid shape.</span></span> <span data-ttu-id="60d26-400">Ein Pyramidenstumpf ist ein 3D-Körper in einer Szene, der relativ zur Viewport-Kamera positioniert ist.</span><span class="sxs-lookup"><span data-stu-id="60d26-400">A viewing frustum is a 3D volume in a scene positioned relative to the viewport's camera.</span></span> <span data-ttu-id="60d26-401">Ein Viewport ist ein 2D-Rechteck, auf das eine 3D-Szene projiziert wird.</span><span class="sxs-lookup"><span data-stu-id="60d26-401">A viewport is a 2D rectangle into which a 3D scene is projected.</span></span> <span data-ttu-id="60d26-402">Weitere Informationen finden Sie unter [Viewports und Zuschneiden](../graphics-concepts/viewports-and-clipping.md).</span><span class="sxs-lookup"><span data-stu-id="60d26-402">For more information, see [Viewports and clipping](../graphics-concepts/viewports-and-clipping.md)</span></span>

<span data-ttu-id="60d26-403">Da das nähergelegene Ende des Ansichtsfrustrums kleiner als das weiter entfernt liegende Ende, hat dies die Wirkung, dass näher bei der Kamera liegende Objekte erweitert werden. So wird Perspektive auf die Szene angewandt.</span><span class="sxs-lookup"><span data-stu-id="60d26-403">Because the near end of the viewing frustum is smaller than the far end, this has the effect of expanding objects that are near to the camera; this is how perspective is applied to the scene.</span></span> <span data-ttu-id="60d26-404">Daher erscheinen Objekte, die näher am Spieler sind, größer; Objekte, die weiter entfernt sind, werden kleiner angezeigt.</span><span class="sxs-lookup"><span data-stu-id="60d26-404">So objects that are closer to the player, appear larger; objects that are further away, appear smaller.</span></span>

<span data-ttu-id="60d26-405">Die Projektionstransformation ist mathematisch gesehen eine Matrix, die in der Regel eine Skalierung und Perspektive verwendet.</span><span class="sxs-lookup"><span data-stu-id="60d26-405">Mathematically, the projection transform is a matrix that is typically both a scale and perspective projection.</span></span> <span data-ttu-id="60d26-406">Sie funktioniert wie die Foto-App einer Kamera.</span><span class="sxs-lookup"><span data-stu-id="60d26-406">It functions like the lens of a camera.</span></span> <span data-ttu-id="60d26-407">Weitere Informationen finden Sie unter [Projektionstransformationen](../graphics-concepts/projection-transform.md).</span><span class="sxs-lookup"><span data-stu-id="60d26-407">For more information, see [Projection transform](../graphics-concepts/projection-transform.md).</span></span>

### <a name="sampler-state"></a><span data-ttu-id="60d26-408">Musterzustand</span><span class="sxs-lookup"><span data-stu-id="60d26-408">Sampler state</span></span>

<span data-ttu-id="60d26-409">Der Musterzustand legt fest, wie Texturdaten verwendet werden mit Textur und deren Modi, Filtern und Genauigkeit.</span><span class="sxs-lookup"><span data-stu-id="60d26-409">Sampler state determines how texture data is sampled using texture addressing modes, filtering, and level of detail.</span></span> <span data-ttu-id="60d26-410">Sampling erfolgt jedes Mal, wenn ein Texturpixel oder Texel aus einer Textur gelesen wird.</span><span class="sxs-lookup"><span data-stu-id="60d26-410">Sampling is done each time a texture pixel, or texel, is read from a texture.</span></span>

<span data-ttu-id="60d26-411">Eine Textur enthält ein Array von Texel oder Texturpixeln.</span><span class="sxs-lookup"><span data-stu-id="60d26-411">A texture contains an array of texels, or texture pixels.</span></span> <span data-ttu-id="60d26-412">Die Position des einzelnen Texels wird von (u, v) festgelegt, wobei u die Breite und v ist die Höhe ist und zwischen 0 und 1 zugeordnet ist die Texturbreite und Höhe abhängig.</span><span class="sxs-lookup"><span data-stu-id="60d26-412">The position of each texel is denoted by (u,v), where u is the width and v is the height, and is mapped between 0 and 1 based on the texture width and height.</span></span> <span data-ttu-id="60d26-413">Die resultierende Texturkoordinaten werden auf einem Texel beim Sampling einer Textur verwendet.</span><span class="sxs-lookup"><span data-stu-id="60d26-413">The resulting texture coordinates are used to address a texel when sampling a texture.</span></span>

<span data-ttu-id="60d26-414">Wenn Texturkoordinaten unter 0 oder 1 sind, definiert der Textur-Adressmodus wie die Texturkoordinate einen den Ort des Texels festlegt.</span><span class="sxs-lookup"><span data-stu-id="60d26-414">When texture coordinates are below 0 or above 1, the texture address mode defines how the texture coordinate addresses a texel location.</span></span> <span data-ttu-id="60d26-415">Bei Verwendung __TextureAddressMode.Clamp__ wird eine Koordinate außerhalb des 0-1-Bereichs auf einen Maximalwert von 1 und Mindestwert von 0 vor dem Sampling festgelegt.</span><span class="sxs-lookup"><span data-stu-id="60d26-415">For example, when using __TextureAddressMode.Clamp__, any coordinate outside the 0-1 range is clamped to a maximum value of 1, and minimum value of 0 before sampling.</span></span>

<span data-ttu-id="60d26-416">Wenn die Textur zu groß oder zu klein für das Polygon ist, wird die Textur gefiltert und dem Abstand angepasst.</span><span class="sxs-lookup"><span data-stu-id="60d26-416">If the texture is too large or too small for the polygon, the texture is filtered to fit the space.</span></span> <span data-ttu-id="60d26-417">Ein Vergrößerungsfilter vergrößert die Textur, ein Verkleinerungsfilter reduziert die Textur, um ihn einem kleineren Bereich anzupassen.</span><span class="sxs-lookup"><span data-stu-id="60d26-417">A magnification filter enlarges a texture, a minification filter reduces the texture to fit into a smaller area.</span></span> <span data-ttu-id="60d26-418">Texturvergrößerung wiederholt die Texel-Beispiel für eine oder mehrere Adressen, was ein verschwommenes Image ergibt.</span><span class="sxs-lookup"><span data-stu-id="60d26-418">Texture magnification repeats the sample texel for one or more addresses which yields a blurrier image.</span></span> <span data-ttu-id="60d26-419">Texturverkleinerung ist komplizierter, da es mehr als einen Texelwert in einen einzelnen Wert kombiniert.</span><span class="sxs-lookup"><span data-stu-id="60d26-419">Texture minification is more complicated because it requires combining more than one texel value into a single value.</span></span> <span data-ttu-id="60d26-420">Dadurch können Aliasing oder ausgefranste Ränder je nach der Texturdaten entstehen.</span><span class="sxs-lookup"><span data-stu-id="60d26-420">This can cause aliasing or jagged edges depending on the texture data.</span></span> <span data-ttu-id="60d26-421">Der am häufigsten verwendete Ansatz für die Verkleinerung ist die Verwendung eine Mipmap.</span><span class="sxs-lookup"><span data-stu-id="60d26-421">The most popular approach for minification is to use a mipmap.</span></span> <span data-ttu-id="60d26-422">Eine Mipmap ist eine Textur mit mehreren Ebenen.</span><span class="sxs-lookup"><span data-stu-id="60d26-422">A mipmap is a multi-level texture.</span></span> <span data-ttu-id="60d26-423">Die Größe der einzelnen Ebenen ist um eine Zweierpotenz kleiner als die vorherigen Ebenen bis zur Textur 1 x 1.</span><span class="sxs-lookup"><span data-stu-id="60d26-423">The size of each level is a power-of-two smaller than the previous level down to a 1x1 texture.</span></span> <span data-ttu-id="60d26-424">Wenn die Verkleinerung verwendet wird, wählt ein Spiel die Mipmap-Ebene, die am der Größe des Renderns am nächsten liegt.</span><span class="sxs-lookup"><span data-stu-id="60d26-424">When minification is used, a game chooses the mipmap level closest to the size that is needed at render time.</span></span> 

### <a name="basicloader"></a><span data-ttu-id="60d26-425">BasicLoader</span><span class="sxs-lookup"><span data-stu-id="60d26-425">BasicLoader</span></span>

<span data-ttu-id="60d26-426">__BasicLoader__ ist eine einfache Loader-Klasse für die Unterstützung beim Laden von Shader, Textur und Gitter von Dateien auf einem Datenträger.</span><span class="sxs-lookup"><span data-stu-id="60d26-426">__BasicLoader__ is a simple loader class that provides support for loading shaders, textures, and meshes from files on disk.</span></span> <span data-ttu-id="60d26-427">Sie bietet synchrone und asynchrone Methoden an.</span><span class="sxs-lookup"><span data-stu-id="60d26-427">It provides both synchronous and asynchronous methods.</span></span> <span data-ttu-id="60d26-428">In diesem Beispielspiel befinden sich die BasicLoader.h/.cpp-Dateien im Ordner __Commons__.</span><span class="sxs-lookup"><span data-stu-id="60d26-428">In this game sample, the BasicLoader.h/.cpp files are found in the __Commons__ folder.</span></span>

<span data-ttu-id="60d26-429">Weitere Informationen finden Sie unter [Basic Loader](complete-code-for-basicloader.md).</span><span class="sxs-lookup"><span data-stu-id="60d26-429">For more information, see [Basic Loader](complete-code-for-basicloader.md).</span></span>